...
| Handler C++ side effects: fall damage, parachute/flight aura interrupts, pet unsummon, sit-to-stand, under-map damage, jump procs | Corregido parcialmente en `#A06.6.1`: Rust ya remueve auras por flags `LandingOrFlight`/`Jump`, levanta al jugador sentado al moverse y registra hooks para unsummon temporal de pet / jump proc. Fall damage, fall info y under-map requieren Map/Pet/Proc runtime real. | OK parcial | `#A06.6.2-#A06.6.4`: fall damage, `UpdateFallInformationIfNeed`, min-height/under-map y conexión real de pet/procs. |
...
- `#A06.6`: `#A06.6.1` corregido; quedan `#A06.6.2-#A06.6.4` para fall damage, under-map y conexión real Pet/Proc.
