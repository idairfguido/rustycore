...
- [ ] **#026k** `wow-entities`/`wow-world`: integrar el bridge en callsites runtime de inventario/dinero/buyback sin perder `UnitData::VirtualItems`. Bloque explícito: los paths de equip/unequip/destroy que actualizan armas no pueden migrar completo hasta portar `VirtualItems` a `wow-entities::UnitData`; los paths sin virtual items deben pasar por `player_values_update_to_update_object`.
