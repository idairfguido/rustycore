...
- [ ] **#021** Migrar `session.rs::tick_creatures_sync`, `send_nearby_creatures` y `handlers/character.rs::update_creature_visibility` a visitors/cell queries del Map.
  ...
  - [x] **#022a** `WorldSession` ya no tiene el campo legacy `visible_creatures`: se reemplaza por `client_visible_guids_like_cpp`, equivalente directo a C++ `Player::m_clientGUIDs`, y las rutas de criaturas/GO actualizan ese set único sin borrar GUIDs de otros tipos. `visible_gameobjects` queda temporalmente porque todavía alimenta ramas GO legacy hasta el visitor completo.
- [ ] **#022** Completar limpieza legacy de `WorldSession`: eliminar `creatures` de tests/fallbacks cuando sus casos estén cubiertos por `MapManager`/objetos canónicos, retirar `visible_gameobjects` cuando el visitor use sólo `client_visible_guids_like_cpp`, y borrar `_attic/` sólo cuando sus tests/avisos útiles estén integrados o descartados explícitamente.
