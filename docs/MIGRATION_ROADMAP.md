...
| `Entities/Item` | wow-entities | ⚠️ | base Item+Bag iniciado; Player inventory/DB/runtime pendiente |
...
- [ ] **#025a** `wow-entities`/`wow-world`: completar ramas dependientes de `Player` real: `GetObjectByTypeMask` para `TYPEMASK_ITEM` vía inventario de Player, `SaveAllPlayers()` real y conexión del accessor al `wow_map::Map` canónico en vez de store bridge interno.
...
- [ ] **#028** `wow-entities`: `Player` from `Entities/Player/`: account/session link, inventory refs, quests, skills, taxi state. Refinado: base `Player` state/setters cerrado en `#028a`, base `Item` state cerrado en `#028b` y base `Bag` state cerrado en `#028c`; siguen pendientes create/load/login, inventario real, quests, skills, taxi, social, mail, group/guild, battleground, visibility and persistence.
