...
- [ ] **#019** Migrar `handlers/combat.rs` y `session.rs::tick_combat_sync` al Map/Entity model.
  ...
  - [ ] **#019b** Completar el port de `Unit::Attack`/`AttackStop`: validaciones C++ completas (`IsValidAttackTarget`, self/dead/world/mounted/GM/evade/vehicle seat), cambios de víctima con `_removeAttacker`/`_addAttacker`, timers/offhand, interrupciones, pets/controlados y side effects de AI.
    ...
    - [x] **#019b.2p** `WorldSession::start_player_attack_like_cpp` alimenta el tramo representable de `CanSeeOrDetect` usando `PhaseShift::can_see` canónico para players/criaturas tipados; targets fuera de fase rechazan el ataque antes de mutar `m_attacking`, como el guard C++ de `WorldObject::IsValidAttackTarget`. Quedan pendientes invisibilidad/stealth/detect/distancia de alerta, que no se simulan.
    ...
    - [x] **#019b.2u** `wow-entities::CombatSubsystem` incorpora el primitivo de `CombatManager::RevalidateCombat()`: recorre refs PvE/PvP, conserva solo las que siguen pasando el predicado equivalente a `CanBeginCombat`, y limpia threat/reverse-threat local de las refs terminadas. El cableado runtime que evalúa `CanBeginCombat` con mapa/fase/friendly/GM sigue pendiente.
    - [ ] **#019b.2** Completar validaciones y side effects no portados: visibilidad avanzada desde runtime completo y revalidación real de `CombatManager`.
