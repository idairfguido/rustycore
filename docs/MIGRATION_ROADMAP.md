...
| `Entities/Item` | wow-entities | ⚠️ | base Item+Bag+Player storage/ObjectAccessor lookup/visible item state iniciado; ownership/DB/runtime pendiente |
...
- [ ] **#025a** `wow-entities`/`wow-world`: completar integración pendiente de `ObjectAccessor`: `SaveAllPlayers()` real y conexión del accessor al `wow_map::Map` canónico en vez de store bridge interno. La rama `GetObjectByTypeMask` para `TYPEMASK_ITEM` vía inventario de Player quedó cerrada en `#028e`.
- [ ] **#026** `wow-packet`/`wow-entities`: Update fields delta from `Entities/Object/Updates/` and `UpdateFields.h`; stop relying on full re-create as normal update path. Refinado: base `UpdateMask` + `UF::ObjectData` delta + writer VALUES de ObjectData cerrado en `#026a`; `UF::ItemData` masks/base values cerrados en `#028b`; `UF::ContainerData` masks/base values cerrados en `#028c`; sigue pendiente generar/portar serializers completos `UnitData`, `PlayerData`, `ActivePlayerData`, `GameObjectData`, `ItemData`, `ContainerData`, etc.
...
- [ ] **#028** `wow-entities`: `Player` from `Entities/Player/`: account/session link, inventory refs, quests, skills, taxi state. Refinado: base `Player` state/setters cerrado en `#028a`, base `Item` state cerrado en `#028b`, base `Bag` state cerrado en `#028c`, storage lookup cerrado en `#028d`, ObjectAccessor item branch cerrado en `#028e` y visible item state cerrado en `#028f`; siguen pendientes create/load/login, inventario real ownership, template-derived item display/binding side effects, quests, skills, taxi, social, mail, group/guild, battleground and persistence.
