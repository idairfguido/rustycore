...
- El gate de access requirements aún no envía los sysmessages/notificaciones exactos de `Player::Satisfy` para `quest_failed_text`, missing-item y level-min cuando C++ no escoge el abort de dificultad; el requisito de achievement de líder ajeno queda conservador hasta portar resolución de líder viva/grupo y el `AchievementMgr` completo sigue pendiente.
