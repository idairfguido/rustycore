...
- `SpellRequiredStoreLikeCpp` y `WorldStatements::SEL_SPELL_REQUIRED` representan `SpellMgr::LoadSpellRequired`: query exacta `SELECT spell_id, req_spell from spell_required`, validación de ambos spells existentes, rechazo de pares en la misma cadena de ranks, skip de duplicado exacto y mapas forward/reverse. Sigue pendiente cargarlo en startup con el chain store real y consumirlo en aprendizaje/cast.
- `SpellLearnSkillStoreLikeCpp` representa `SpellMgr::LoadSpellLearnSkills`: no usa SQL ni DB2 propios en el C++ actual; deriva `mSpellLearnSkills` desde `mSpellInfoMap`, salta dificultades no base, toma el primer efecto `SPELL_EFFECT_SKILL` o `SPELL_EFFECT_DUAL_WIELD`, usa `CalcValue()` para `step` y expone `GetSpellLearnSkill`. Sigue pendiente cablearlo al SpellMgr vivo con el `SpellInfo` completo.
- `SpellLearnSpellStoreLikeCpp` y `WorldStatements::SEL_SPELL_LEARN_SPELL` representan `SpellMgr::LoadSpellLearnSpells`: query exacta `SELECT entry, SpellID, Active FROM spell_learn_spell`, validación SQL de spell fuente/destino, skip de fuentes talent, copia de rows SQL para detectar redundancia con SpellInfo/SpellLearnSpell.db2, early-return C++ cuando la tabla SQL está vacía, `Active`/`AutoLearned` y accessors tipo multimap. Sigue pendiente cablearlo al SpellMgr vivo y consumirlo en cascadas de aprender/desaprender spells.
- `SpellGroupStoreLikeCpp` y `WorldStatements::SEL_SPELL_GROUP` representan `SpellMgr::LoadSpellGroups`: query exacta `SELECT id, spell_id FROM spell_group`, rechazo de ids DB en rango core sin definición, validación de spells existentes y first-rank, referencias negativas a grupos anidados, expansión recursiva con protección de ciclos y mapas spell→group/group→spell. Sigue pendiente cablearlo al SpellMgr vivo y usarlo junto a `spell_group_stack_rules`.
- `SpellGroupStackRuleStoreLikeCpp` y `WorldStatements::SEL_SPELL_GROUP_STACK_RULES` representan `SpellMgr::LoadSpellGroupStackRules`: query exacta `SELECT group_id, stack_rule FROM spell_group_stack_rules`, validación de reglas/grupos, mapa `mSpellGroupStack`, inferencia C++ de `SPELL_GROUP_STACK_RULE_EXCLUSIVE_SAME_EFFECT` con el subgrupo haste melee/ranged y accessor representado de `CheckSpellGroupStackRules`. Sigue pendiente cablearlo al SpellMgr vivo y al runtime real de auras.
- `SpellProcStoreLikeCpp` y `WorldStatements::SEL_SPELL_PROC` representan la primera mitad explícita de `SpellMgr::LoadSpellProcs`: query exacta de 18 columnas, rows SQL → `SpellProcEntry`, `SpellId` negativo expandido por rank chain, defaults desde `SpellInfo` (`ProcFlags`, charges, chance, cooldown) y validaciones/saneamientos principales. Sigue pendiente la segunda mitad C++ que genera proc entries implícitos desde auras, el fallback de dificultad y todo el runtime de `ProcEventInfo`/auras.
