...
- [ ] **#PETS.18** Implement `Pet::cast_pet_auras(current)` + `Pet::cast_pet_aura(petAura)` + `Pet::is_pet_aura(aura)` — represented decision helper exists in `wow-entities` (`PetAuraLikeCpp`) and represented `spell_pet_auras` loader/store now lives in `wow-data` (`SpellPetAuraStoreLikeCpp`; wildcard `petEntry=0`, Demonic Knowledge basepoint), but live startup wiring into `SpellMgr` and real `CastSpell` execution are still pending (M)
