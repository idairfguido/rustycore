...
- [ ] **#PETS.18** Implement `Pet::cast_pet_auras(current)` + `Pet::cast_pet_aura(petAura)` + `Pet::is_pet_aura(aura)` — represented decision helper exists (`PetAuraLikeCpp` plan, wildcard `petEntry=0`, Demonic Knowledge basepoint), but live `spell_pet_auras` cache and real `CastSpell` execution are still pending (M)
