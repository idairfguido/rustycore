...
978 rows total. Breakdown by `status` column:
...
| `represented-partial` | 326 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **326 of the 956 "addressed" rows (34.10%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.76%** (958/980) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **64.18%** (629/980) | items with no open boundary |
...
1. **`97.75%` measures "represented game logic addressed", not "gap-free server."**
...
2. **This is ONE miniphase (R8-entities).** The full port also has the r7-l1/l2/l3
   infra/packets/maps miniphases and more. 980 R8 rows are not "the whole server".

## Honest one-line status

The bulk of the game logic is ported and contrasted against C++ in a per-session
"represented" model (~64.18% with no declared partial gaps, ~97.76% of inventory rows touched).
