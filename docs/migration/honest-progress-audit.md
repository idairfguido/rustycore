...
**Date:** 2026-06-14 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.849`
...
843 rows total. Breakdown by `status` column:

| status | rows | meaning |
|---|---:|---|
| `complete` | 419 | genuinely complete |
| `represented-complete` | 159 | complete **within the "represented" per-session model** (not live runtime) |
...
- **223 of the 821 "addressed" rows (27.2%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.39%** (821/843) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **70.82%** (597/843) | items with no open boundary |
