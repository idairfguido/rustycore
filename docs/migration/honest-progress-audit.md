...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.905`
...
897 rows total. Breakdown by `status` column:
...
| `represented-partial` | 249 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **249 of the 875 "addressed" rows (28.5%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.55%** (875/897) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **69.68%** (625/897) | items with no open boundary |
...
"represented" model (~69.68% with no declared partial gaps, ~97.6% of inventory rows touched).
