...
**Date:** 2026-06-13 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.844`
...
838 rows total. Breakdown by `status` column:
...
- **223 of the 816 "addressed" rows (27.3%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.37%** (816/838) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **70.64%** (592/838) | items with no open boundary |
