...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.916`
...
908 rows total. Breakdown by `status` column:
...
| `represented-partial` | 260 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **260 of the 886 "addressed" rows (29.3%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.58%** (886/908) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **68.83%** (625/908) | items with no open boundary |
...
"represented" model (~68.83% with no declared partial gaps, ~97.6% of inventory rows touched).
