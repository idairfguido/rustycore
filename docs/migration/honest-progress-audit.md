...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.933`

This document exists to prevent the headline `97.62%` from being read as "almost a
...
925 rows total. Breakdown by `status` column:
...
| `represented-partial` | 275 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **275 of the 903 "addressed" rows (30.5%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.62%** (903/925) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **67.78%** (627/925) | items with no open boundary |
...
"represented" model (~67.78% with no declared partial gaps, ~97.62% of inventory rows touched).
