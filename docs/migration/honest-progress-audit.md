...
**Date:** 2026-06-19 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.1115`
...
1107 rows total. Breakdown by `status` column:
...
| `represented-partial` | 421 | **partial — carries documented open boundaries (gaps)** |
...
- **456 of the 1085 "addressed" rows (42.03%) are partial-boundary rows** (`represented-partial`, `bugfix-partial`, or `partial`) — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **98.01%** (1085/1107) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **56.82%** (629/1107) | items with no open boundary |
...
"represented" model (~56.82% with no declared partial gaps, ~98.01% of inventory rows touched).
