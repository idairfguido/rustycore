...
**Date:** 2026-06-13 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.828`

This document exists to prevent the headline `97.32%` from being read as "almost a
...
822 rows total. Breakdown by `status` column:

| status | rows | meaning |
|---|---:|---|
| `complete` | 419 | genuinely complete |
| `represented-complete` | 141 | complete **within the "represented" per-session model** (not live runtime) |
...
- **220 of the 800 "addressed" rows (27.5%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.32%** (800/822) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **70.32%** (578/822) | items with no open boundary |
...
1. **`97.32%` measures "represented game logic addressed", not "gap-free server."**
