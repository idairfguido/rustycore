...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.931`
...
923 rows total. Breakdown by `status` column:
...
| `represented-partial` | 273 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **273 of the 901 "addressed" rows (30.3%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.62%** (901/923) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **67.93%** (627/923) | items with no open boundary |
...
"represented" model (~67.93% with no declared partial gaps, ~97.62% of inventory rows touched).
