...
**Date:** 2026-06-20 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.1183`
...
1175 rows total. Breakdown by `status` column:
...
| `represented-partial` | 488 | **partial — carries documented open boundaries (gaps)** |
...
- **524 of the 1153 "addressed" rows (45.45%) are partial-boundary rows** (`represented-partial`, `bugfix-partial`, or `partial`) — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **98.13%** (1153/1175) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **53.53%** (629/1175) | items with no open boundary |
...
"represented" model (~53.53% with no declared partial gaps, ~98.13% of inventory rows touched).
