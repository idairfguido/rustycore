...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.912`
...
904 rows total. Breakdown by `status` column:
...
| `represented-partial` | 256 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **256 of the 882 "addressed" rows (29.0%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.57%** (882/904) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **69.14%** (625/904) | items with no open boundary |
...
"represented" model (~69.14% with no declared partial gaps, ~97.6% of inventory rows touched).
