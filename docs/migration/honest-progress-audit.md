...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.983`
...
975 rows total. Breakdown by `status` column:
...
| `represented-partial` | 323 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **323 of the 953 "addressed" rows (33.89%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.74%** (953/975) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **64.51%** (629/975) | items with no open boundary |
...
"represented" model (~64.51% with no declared partial gaps, ~97.74% of inventory rows touched).
