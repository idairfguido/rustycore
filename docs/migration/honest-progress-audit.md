...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.908`

This document exists to prevent the headline `97.56%` from being read as "almost a
...
900 rows total. Breakdown by `status` column:
...
| `represented-partial` | 252 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **252 of the 878 "addressed" rows (28.7%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.56%** (878/900) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **69.44%** (625/900) | items with no open boundary |
...
1. **`97.56%` measures "represented game logic addressed", not "gap-free server."**
...
"represented" model (~69.44% with no declared partial gaps, ~97.6% of inventory rows touched).
