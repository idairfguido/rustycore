...
**Date:** 2026-06-19 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.1155`
...
1147 rows total. Breakdown by `status` column:
...
| `represented-partial` | 461 | **partial — carries documented open boundaries (gaps)** |
...
- **496 of the 1125 "addressed" rows (44.09%) are partial-boundary rows** (`represented-partial`, `bugfix-partial`, or `partial`) — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **98.08%** (1125/1147) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **54.84%** (629/1147) | items with no open boundary |
...
"represented" model (~54.84% with no declared partial gaps, ~98.08% of inventory rows touched).
