...
**Date:** 2026-06-13 · **Base commit:** `824e3261` (`develop` after `#NEXT.R8.ENTITIES.821`)

This document exists to prevent the headline `97.30%` from being read as "almost a
...
815 rows total. Breakdown by `status` column:

| status | rows | meaning |
|---|---:|---|
| `complete` | 419 | genuinely complete |
| `represented-complete` | 135 | complete **within the "represented" per-session model** (not live runtime) |
| `represented-partial` | 219 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **219 of the 793 "addressed" rows (27.6%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.30%** (793/815) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented-complete` + `reviewed-validated`) | **70.06%** (571/815) | items with no open boundary |
...
1. **`97.30%` measures "represented game logic addressed", not "gap-free server."**
