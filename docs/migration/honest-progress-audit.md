...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.952`

This document exists to prevent the headline `97.88%` from being read as "almost a
...
942 rows total. Breakdown by `status` column:

| status | rows | meaning |
|---|---:|---|
| `complete` | 419 | genuinely complete |
| `represented-complete` | 190 | complete **within the "represented" per-session model** (not live runtime) |
| `represented-partial` | 293 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 20 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **293 of the 922 "addressed" rows (31.78%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.88%** (922/942) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **66.67%** (628/942) | items with no open boundary |
...
1. **`97.88%` measures "represented game logic addressed", not "gap-free server."**
...
2. **This is ONE miniphase (R8-entities).** The full port also has the r7-l1/l2/l3
   infra/packets/maps miniphases and more. 942 R8 rows are not "the whole server".

## Honest one-line status

The bulk of the game logic is ported and contrasted against C++ in a per-session
"represented" model (~66.67% with no declared partial gaps, ~97.88% of inventory rows touched).
