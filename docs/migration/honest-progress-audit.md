...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.950`
...
| `represented-partial` | 291 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **291 of the 920 "addressed" rows (31.63%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.66%** (920/942) | the headline number — real but generous |
...
"represented" model (~66.67% with no declared partial gaps, ~97.66% of inventory rows touched).
