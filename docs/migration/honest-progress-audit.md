...
**Date:** 2026-06-20 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.1182`

This document exists to prevent the headline `98.13%` from being read as "almost a
...
1174 rows total. Breakdown by `status` column:
...
| `represented-partial` | 487 | **partial — carries documented open boundaries (gaps)** |
...
- **523 of the 1152 "addressed" rows (45.40%) are partial-boundary rows** (`represented-partial`, `bugfix-partial`, or `partial`) — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **98.13%** (1152/1174) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **53.58%** (629/1174) | items with no open boundary |
...
"represented" model (~53.58% with no declared partial gaps, ~98.13% of inventory rows touched).
