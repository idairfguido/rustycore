...
**Date:** 2026-06-19 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.1143`
...
1135 rows total. Breakdown by `status` column:
...
| `represented-partial` | 449 | **partial — carries documented open boundaries (gaps)** |
...
- **484 of the 1113 "addressed" rows (43.49%) are partial-boundary rows** (`represented-partial`, `bugfix-partial`, or `partial`) — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **98.06%** (1113/1135) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **55.42%** (629/1135) | items with no open boundary |
...
"represented" model (~55.42% with no declared partial gaps, ~98.06% of inventory rows touched).
