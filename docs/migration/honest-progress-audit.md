...
**Date:** 2026-06-14 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.883`

This document exists to prevent the headline `97.49%` from being read as "almost a
...
877 rows total. Breakdown by `status` column:

| status | rows | meaning |
|---|---:|---|
| `complete` | 419 | genuinely complete |
| `represented-complete` | 177 | complete **within the "represented" per-session model** (not live runtime) |
| `represented-partial` | 239 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **239 of the 855 "addressed" rows (28.0%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.49%** (855/877) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **70.13%** (615/877) | items with no open boundary |
...
1. **`97.49%` measures "represented game logic addressed", not "gap-free server."**
...
2. **This is ONE miniphase (R8-entities).** The full port also has the r7-l1/l2/l3
   infra/packets/maps miniphases and more. 875 is not "the whole server".
