...
**Date:** 2026-06-19 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.1140`
...
1132 rows total. Breakdown by `status` column:
...
| `represented-partial` | 446 | **partial — carries documented open boundaries (gaps)** |
...
- **481 of the 1110 "addressed" rows (43.33%) are partial-boundary rows** (`represented-partial`, `bugfix-partial`, or `partial`) — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **98.06%** (1110/1132) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **55.57%** (629/1132) | items with no open boundary |
...
"represented" model (~55.57% with no declared partial gaps, ~98.06% of inventory rows touched).
