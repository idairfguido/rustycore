...
**Date:** 2026-06-13 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.823`
...
817 rows total. Breakdown by `status` column:

| status | rows | meaning |
|---|---:|---|
| `complete` | 419 | genuinely complete |
| `represented-complete` | 137 | complete **within the "represented" per-session model** (not live runtime) |
...
- **219 of the 795 "addressed" rows (27.5%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.31%** (795/817) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented-complete` + `reviewed-validated`) | **70.13%** (573/817) | items with no open boundary |
