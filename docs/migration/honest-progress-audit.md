...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.953`
...
945 rows total. Breakdown by `status` column:
...
| `represented-partial` | 294 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **294 of the 923 "addressed" rows (31.85%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.67%** (923/945) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **66.46%** (628/945) | items with no open boundary |
...
"represented" model (~66.46% with no declared partial gaps, ~97.67% of inventory rows touched).
