...
**Date:** 2026-06-19 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.1114`
...
1106 rows total. Breakdown by `status` column:
...
| `represented-partial` | 420 | **partial — carries documented open boundaries (gaps)** |
...
- **455 of the 1084 "addressed" rows (41.97%) are partial-boundary rows** (`represented-partial`, `bugfix-partial`, or `partial`) — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **98.01%** (1084/1106) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **56.87%** (629/1106) | items with no open boundary |
...
"represented" model (~56.87% with no declared partial gaps, ~98.01% of inventory rows touched).
