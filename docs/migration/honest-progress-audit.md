...
**Date:** 2026-06-19 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.1105`
...
1097 rows total. Breakdown by `status` column:
...
| `represented-partial` | 411 | **partial — carries documented open boundaries (gaps)** |
...
- **446 of the 1075 "addressed" rows (41.49%) are partial-boundary rows** (`represented-partial`, `bugfix-partial`, or `partial`) — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.99%** (1075/1097) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **57.34%** (629/1097) | items with no open boundary |
...
"represented" model (~57.34% with no declared partial gaps, ~97.99% of inventory rows touched).
