...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.928`
...
920 rows total. Breakdown by `status` column:
...
| `represented-partial` | 270 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **270 of the 898 "addressed" rows (30.1%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.61%** (898/920) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **68.15%** (627/920) | items with no open boundary |
...
"represented" model (~68.15% with no declared partial gaps, ~97.61% of inventory rows touched).
