...
**Date:** 2026-06-19 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.1141`

This document exists to prevent the headline `98.05%` from being read as "almost a
...
1133 rows total. Breakdown by `status` column:
...
| `represented-partial` | 447 | **partial — carries documented open boundaries (gaps)** |
...
- **482 of the 1111 "addressed" rows (43.38%) are partial-boundary rows** (`represented-partial`, `bugfix-partial`, or `partial`) — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **98.06%** (1111/1133) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **55.52%** (629/1133) | items with no open boundary |
...
2. **This is ONE miniphase (R8-entities).** The full port also has the r7-l1/l2/l3
   infra/packets/maps miniphases and more. 1125 R8 rows are not "the whole server".

## Honest one-line status

The bulk of the game logic is ported and contrasted against C++ in a per-session
"represented" model (~55.52% with no declared partial gaps, ~98.06% of inventory rows touched).
