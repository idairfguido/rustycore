...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.915`

This document exists to prevent the headline `97.57%` from being read as "almost a
...
907 rows total. Breakdown by `status` column:
...
| `represented-partial` | 259 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **259 of the 885 "addressed" rows (29.3%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.57%** (885/907) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **68.91%** (625/907) | items with no open boundary |
...
1. **`97.57%` measures "represented game logic addressed", not "gap-free server."**
...
"represented" model (~68.91% with no declared partial gaps, ~97.6% of inventory rows touched).
