...
**Date:** 2026-06-19 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.1154`
...
1146 rows total. Breakdown by `status` column:
...
| `represented-partial` | 460 | **partial — carries documented open boundaries (gaps)** |
...
- **495 of the 1124 "addressed" rows (44.04%) are partial-boundary rows** (`represented-partial`, `bugfix-partial`, or `partial`) — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **98.08%** (1124/1146) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **54.89%** (629/1146) | items with no open boundary |
...
"represented" model (~54.89% with no declared partial gaps, ~98.08% of inventory rows touched).
