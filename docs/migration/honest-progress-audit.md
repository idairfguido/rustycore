...
**Date:** 2026-06-13 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.835`

This document exists to prevent the headline `97.35%` from being read as "almost a
...
829 rows total. Breakdown by `status` column:

| status | rows | meaning |
|---|---:|---|
| `complete` | 419 | genuinely complete |
| `represented-complete` | 147 | complete **within the "represented" per-session model** (not live runtime) |
...
- **221 of the 807 "addressed" rows (27.4%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.35%** (807/829) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **70.57%** (585/829) | items with no open boundary |
...
1. **`97.35%` measures "represented game logic addressed", not "gap-free server."**
