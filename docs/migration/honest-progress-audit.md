...
**Date:** 2026-06-19 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.1153`
...
1145 rows total. Breakdown by `status` column:
...
| `represented-partial` | 459 | **partial — carries documented open boundaries (gaps)** |
...
- **494 of the 1123 "addressed" rows (43.99%) are partial-boundary rows** (`represented-partial`, `bugfix-partial`, or `partial`) — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **98.08%** (1123/1145) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **54.93%** (629/1145) | items with no open boundary |
...
"represented" model (~54.93% with no declared partial gaps, ~98.08% of inventory rows touched).
