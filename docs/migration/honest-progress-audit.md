...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.955`

This document exists to prevent the headline `97.67%` from being read as "almost a
...
947 rows total. Breakdown by `status` column:
...
| `represented-partial` | 296 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **296 of the 925 "addressed" rows (32.00%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.68%** (925/947) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **66.31%** (628/947) | items with no open boundary |
...
1. **`97.67%` measures "represented game logic addressed", not "gap-free server."**
...
"represented" model (~66.31% with no declared partial gaps, ~97.68% of inventory rows touched).
