...
**Date:** 2026-06-14 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.890`

This document exists to prevent the headline `97.50%` from being read as "almost a
...
882 rows total. Breakdown by `status` column:

| status | rows | meaning |
|---|---:|---|
| `complete` | 419 | genuinely complete |
| `represented-complete` | 184 | complete **within the "represented" per-session model** (not live runtime) |
| `represented-partial` | 237 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **237 of the 860 "addressed" rows (27.6%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.51%** (860/882) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **70.52%** (622/882) | items with no open boundary |
