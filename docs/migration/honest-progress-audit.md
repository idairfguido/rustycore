...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.924`
...
916 rows total. Breakdown by `status` column:
...
| `represented-partial` | 267 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **267 of the 894 "addressed" rows (29.9%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.60%** (894/916) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **68.34%** (626/916) | items with no open boundary |
...
"represented" model (~68.34% with no declared partial gaps, ~97.6% of inventory rows touched).
