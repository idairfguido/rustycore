...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.962`

This document exists to prevent the headline `97.69%` from being read as "almost a
...
954 rows total. Breakdown by `status` column:
...
| `represented-partial` | 303 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **303 of the 932 "addressed" rows (32.51%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.69%** (932/954) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **65.83%** (628/954) | items with no open boundary |
...
1. **`97.69%` measures "represented game logic addressed", not "gap-free server."**
...
2. **This is ONE miniphase (R8-entities).** The full port also has the r7-l1/l2/l3
   infra/packets/maps miniphases and more. 954 R8 rows are not "the whole server".

## Honest one-line status

The bulk of the game logic is ported and contrasted against C++ in a per-session
"represented" model (~65.83% with no declared partial gaps, ~97.69% of inventory rows touched).
