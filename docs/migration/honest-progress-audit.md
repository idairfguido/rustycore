...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.900`
...
892 rows total. Breakdown by `status` column:
...
- **246 of the 870 "addressed" rows (28.3%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.53%** (870/892) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **69.84%** (623/892) | items with no open boundary |
...
"represented" model (~69.8% with no declared partial gaps, ~97.5% of inventory rows touched).
