...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.910`
...
902 rows total. Breakdown by `status` column:
...
| `represented-partial` | 254 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **254 of the 880 "addressed" rows (28.9%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.56%** (880/902) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **69.29%** (625/902) | items with no open boundary |
...
"represented" model (~69.29% with no declared partial gaps, ~97.6% of inventory rows touched).
