...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.913`
...
905 rows total. Breakdown by `status` column:
...
| `represented-partial` | 257 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **257 of the 883 "addressed" rows (29.1%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.57%** (883/905) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **69.06%** (625/905) | items with no open boundary |
...
"represented" model (~69.06% with no declared partial gaps, ~97.6% of inventory rows touched).
