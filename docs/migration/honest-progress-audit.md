...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.934`
...
926 rows total. Breakdown by `status` column:
...
| `represented-partial` | 276 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **276 of the 904 "addressed" rows (30.5%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.62%** (904/926) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **67.71%** (627/926) | items with no open boundary |
...
"represented" model (~67.71% with no declared partial gaps, ~97.62% of inventory rows touched).
