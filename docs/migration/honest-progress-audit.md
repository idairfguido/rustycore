...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.907`
...
899 rows total. Breakdown by `status` column:
...
| `represented-partial` | 251 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **251 of the 877 "addressed" rows (28.6%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.55%** (877/899) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **69.52%** (625/899) | items with no open boundary |
...
"represented" model (~69.52% with no declared partial gaps, ~97.6% of inventory rows touched).
