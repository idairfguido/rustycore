...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.932`
...
924 rows total. Breakdown by `status` column:
...
| `represented-partial` | 274 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **274 of the 902 "addressed" rows (30.4%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.62%** (902/924) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **67.86%** (627/924) | items with no open boundary |
...
"represented" model (~67.86% with no declared partial gaps, ~97.62% of inventory rows touched).
