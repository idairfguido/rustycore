...
**Date:** 2026-06-13 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.832`
...
826 rows total. Breakdown by `status` column:

| status | rows | meaning |
|---|---:|---|
| `complete` | 419 | genuinely complete |
| `represented-complete` | 144 | complete **within the "represented" per-session model** (not live runtime) |
...
- **221 of the 804 "addressed" rows (27.5%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.34%** (804/826) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **70.34%** (581/826) | items with no open boundary |
