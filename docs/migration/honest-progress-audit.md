...
**Date:** 2026-06-19 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.1142`
...
1134 rows total. Breakdown by `status` column:
...
| `represented-partial` | 448 | **partial — carries documented open boundaries (gaps)** |
...
- **483 of the 1112 "addressed" rows (43.44%) are partial-boundary rows** (`represented-partial`, `bugfix-partial`, or `partial`) — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **98.06%** (1112/1134) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **55.47%** (629/1134) | items with no open boundary |
...
"represented" model (~55.47% with no declared partial gaps, ~98.06% of inventory rows touched).
