...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.941`
...
933 rows total. Breakdown by `status` column:
...
| `represented-partial` | 283 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **283 of the 911 "addressed" rows (31.06%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.64%** (911/933) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **67.20%** (627/933) | items with no open boundary |
...
"represented" model (~67.20% with no declared partial gaps, ~97.64% of inventory rows touched).
