...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.956`
...
948 rows total. Breakdown by `status` column:
...
| `represented-partial` | 297 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **297 of the 926 "addressed" rows (32.07%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.68%** (926/948) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **66.24%** (628/948) | items with no open boundary |
...
"represented" model (~66.24% with no declared partial gaps, ~97.68% of inventory rows touched).
