...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.930`
...
922 rows total. Breakdown by `status` column:
...
| `represented-partial` | 272 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **272 of the 900 "addressed" rows (30.2%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.61%** (900/922) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **68.00%** (627/922) | items with no open boundary |
...
"represented" model (~68.00% with no declared partial gaps, ~97.61% of inventory rows touched).
