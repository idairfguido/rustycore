...
**Date:** 2026-06-19 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.1124`
...
1116 rows total. Breakdown by `status` column:
...
| `represented-partial` | 430 | **partial — carries documented open boundaries (gaps)** |
...
- **465 of the 1094 "addressed" rows (42.50%) are partial-boundary rows** (`represented-partial`, `bugfix-partial`, or `partial`) — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **98.03%** (1094/1116) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **56.36%** (629/1116) | items with no open boundary |
...
"represented" model (~56.36% with no declared partial gaps, ~98.03% of inventory rows touched).
