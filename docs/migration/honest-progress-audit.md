...
**Date:** 2026-06-13 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.830`

This document exists to prevent the headline `97.33%` from being read as "almost a
...
824 rows total. Breakdown by `status` column:

| status | rows | meaning |
|---|---:|---|
| `complete` | 419 | genuinely complete |
| `represented-complete` | 143 | complete **within the "represented" per-session model** (not live runtime) |
| `represented-partial` | 220 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **220 of the 802 "addressed" rows (27.4%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.33%** (802/824) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **70.39%** (580/824) | items with no open boundary |
...
1. **`97.33%` measures "represented game logic addressed", not "gap-free server."**
