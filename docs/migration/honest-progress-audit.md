...
> `#NEXT.R8.ENTITIES.1059` are now recorded; a full percentage refresh still
...
1051 rows total. Breakdown by `status` column:
...
| `bugfix-partial` | 26 | bugfix slice with explicit remaining boundaries |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **400 of the 1029 "addressed" rows (38.87%) are partial-boundary rows** (`represented-partial`, `bugfix-partial`, or `partial`) — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.91%** (1029/1051) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **59.85%** (629/1051) | items with no open boundary |
...
"represented" model (~59.96% with no declared partial gaps, ~97.90% of inventory rows touched).
