...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.939`
...
931 rows total. Breakdown by `status` column:
...
| `represented-partial` | 281 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **281 of the 909 "addressed" rows (30.91%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.64%** (909/931) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **67.35%** (627/931) | items with no open boundary |
...
"represented" model (~67.35% with no declared partial gaps, ~97.64% of inventory rows touched).
