...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.904`
...
896 rows total. Breakdown by `status` column:
...
| `represented-partial` | 248 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **248 of the 874 "addressed" rows (28.4%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.54%** (874/896) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **69.75%** (625/896) | items with no open boundary |
...
"represented" model (~69.75% with no declared partial gaps, ~97.5% of inventory rows touched).
