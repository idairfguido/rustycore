...
> `#NEXT.R8.ENTITIES.1060` are now recorded; a full percentage refresh still
...
1052 rows total. Breakdown by `status` column:
...
| `bugfix-partial` | 27 | bugfix slice with explicit remaining boundaries |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **401 of the 1030 "addressed" rows (38.93%) are partial-boundary rows** (`represented-partial`, `bugfix-partial`, or `partial`) — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.91%** (1030/1052) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **59.79%** (629/1052) | items with no open boundary |
...
"represented" model (~59.79% with no declared partial gaps, ~97.91% of inventory rows touched).
