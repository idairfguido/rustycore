...
**Date:** 2026-06-13 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.824`

This document exists to prevent the headline `97.31%` from being read as "almost a
...
818 rows total. Breakdown by `status` column:

| status | rows | meaning |
|---|---:|---|
| `complete` | 419 | genuinely complete |
| `represented-complete` | 138 | complete **within the "represented" per-session model** (not live runtime) |
...
- **219 of the 796 "addressed" rows (27.5%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.31%** (796/818) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented-complete` + `reviewed-validated`) | **70.17%** (574/818) | items with no open boundary |
...
1. **`97.31%` measures "represented game logic addressed", not "gap-free server."**
