...
**Date:** 2026-06-19 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.1144`

This document exists to prevent the headline `98.06%` from being read as "almost a
...
1136 rows total. Breakdown by `status` column:
...
| `represented-partial` | 450 | **partial — carries documented open boundaries (gaps)** |
...
- **485 of the 1114 "addressed" rows (43.54%) are partial-boundary rows** (`represented-partial`, `bugfix-partial`, or `partial`) — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **98.06%** (1114/1136) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **55.37%** (629/1136) | items with no open boundary |
...
2. **This is ONE miniphase (R8-entities).** The full port also has the r7-l1/l2/l3
   infra/packets/maps miniphases and more. 1135 R8 rows are not "the whole server".

## Honest one-line status

The bulk of the game logic is ported and contrasted against C++ in a per-session
"represented" model (~55.37% with no declared partial gaps, ~98.06% of inventory rows touched).
