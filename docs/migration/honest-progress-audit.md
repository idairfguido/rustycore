...
**Date:** 2026-06-19 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.1172`

This document exists to prevent the headline `98.11%` from being read as "almost a
...
1164 rows total. Breakdown by `status` column:
...
| `represented-partial` | 477 | **partial — carries documented open boundaries (gaps)** |
...
- **513 of the 1142 "addressed" rows (44.92%) are partial-boundary rows** (`represented-partial`, `bugfix-partial`, or `partial`) — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **98.11%** (1142/1164) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **54.04%** (629/1164) | items with no open boundary |
...
1. **`98.11%` measures "represented game logic addressed", not "gap-free server."**
...
2. **This is ONE miniphase (R8-entities).** The full port also has the r7-l1/l2/l3
   infra/packets/maps miniphases and more. 1164 R8 rows are not "the whole server".

## Honest one-line status

The bulk of the game logic is ported and contrasted against C++ in a per-session
"represented" model (~54.04% with no declared partial gaps, ~98.11% of inventory rows touched).
