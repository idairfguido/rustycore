...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.951`

This document exists to prevent the headline `97.77%` from being read as "almost a
...
| `represented-partial` | 292 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 21 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **292 of the 921 "addressed" rows (31.70%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.77%** (921/942) | the headline number — real but generous |
...
1. **`97.77%` measures "represented game logic addressed", not "gap-free server."**
...
"represented" model (~66.67% with no declared partial gaps, ~97.77% of inventory rows touched).
