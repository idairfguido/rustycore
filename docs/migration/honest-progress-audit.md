...
**Date:** 2026-06-13 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.825`
...
819 rows total. Breakdown by `status` column:
...
- **220 of the 797 "addressed" rows (27.6%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.31%** (797/819) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented-complete` + `reviewed-validated`) | **70.09%** (574/819) | items with no open boundary |
