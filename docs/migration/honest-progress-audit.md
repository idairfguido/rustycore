...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.954`
...
946 rows total. Breakdown by `status` column:
...
| `represented-partial` | 295 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **295 of the 924 "addressed" rows (31.93%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.67%** (924/946) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **66.38%** (628/946) | items with no open boundary |
...
"represented" model (~66.38% with no declared partial gaps, ~97.67% of inventory rows touched).
