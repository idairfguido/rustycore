...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.925`
...
917 rows total. Breakdown by `status` column:
...
| `represented-partial` | 268 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **268 of the 895 "addressed" rows (29.9%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.60%** (895/917) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **68.27%** (626/917) | items with no open boundary |
...
"represented" model (~68.27% with no declared partial gaps, ~97.6% of inventory rows touched).
