...
**Date:** 2026-06-19 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.1135`
...
1127 rows total. Breakdown by `status` column:
...
| `represented-partial` | 441 | **partial — carries documented open boundaries (gaps)** |
...
- **476 of the 1105 "addressed" rows (43.08%) are partial-boundary rows** (`represented-partial`, `bugfix-partial`, or `partial`) — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **98.05%** (1105/1127) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **55.81%** (629/1127) | items with no open boundary |
...
"represented" model (~55.81% with no declared partial gaps, ~98.05% of inventory rows touched).
