...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.927`

This document exists to prevent the headline `97.61%` from being read as "almost a
...
919 rows total. Breakdown by `status` column:
...
- **269 of the 897 "addressed" rows (30.0%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.61%** (897/919) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **68.23%** (627/919) | items with no open boundary |
...
"represented" model (~68.23% with no declared partial gaps, ~97.61% of inventory rows touched).
