...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.982`
...
974 rows total. Breakdown by `status` column:
...
| `represented-partial` | 322 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **322 of the 952 "addressed" rows (33.82%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.74%** (952/974) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **64.58%** (629/974) | items with no open boundary |
...
"represented" model (~64.58% with no declared partial gaps, ~97.74% of inventory rows touched).
