...
**Date:** 2026-06-15 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.942`
...
934 rows total. Breakdown by `status` column:
...
| `represented-partial` | 284 | **partial — carries documented open boundaries (gaps)** |
| `reviewed-validated` | 17 | validated |
| `pending` | 22 | not started |
| other (`partial` / `represented` / `test-fixture-unblock`) | 3 | — |

- **284 of the 912 "addressed" rows (31.14%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.64%** (912/934) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **67.13%** (627/934) | items with no open boundary |
...
"represented" model (~67.13% with no declared partial gaps, ~97.64% of inventory rows touched).
