...
**Date:** 2026-06-13 · **Base commit:** `develop` after `#NEXT.R8.ENTITIES.834`

This document exists to prevent the headline `97.34%` from being read as "almost a
...
828 rows total. Breakdown by `status` column:

| status | rows | meaning |
|---|---:|---|
| `complete` | 419 | genuinely complete |
| `represented-complete` | 146 | complete **within the "represented" per-session model** (not live runtime) |
...
- **221 of the 806 "addressed" rows (27.4%) are `represented-partial`** — each has open boundaries by definition.
...
|---|---:|---|
| Items "addressed" (not `pending`) | **97.34%** (806/828) | the headline number — real but generous |
| No declared partial gaps (`complete` + `represented` + `represented-complete` + `reviewed-validated` + `test-fixture-unblock`) | **70.53%** (584/828) | items with no open boundary |
...
1. **`97.34%` measures "represented game logic addressed", not "gap-free server."**
