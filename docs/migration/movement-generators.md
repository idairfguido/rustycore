...
- `#A06.8h.3e.10` connects represented creature distract/rotate to real facing-only `MoveSplineInit` launches (`MoveTo(current position)` + `SetFacing(angle)`), preserves C++'s stationary facing spline shape, mutates real `UnitData::StandState` to stand on distract initialize, applies distract final home orientation, and returns rotate `MovementInform(ROTATE,id)` as a represented payload.
- Still missing: generalized executable generator behavior against a real `Unit`, pathgen-backed destinations, real `MovementInform` dispatch, real `CallAssistance` map/AI effects, follow/chase/flee/random/waypoint/taxi/spline-chain runtime logic, and generic MotionMaster delayed-action semantics.
