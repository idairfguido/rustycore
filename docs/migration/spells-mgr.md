...
3. Several SQL/DB2 spell loaders are still absent or only represented. `LoadSpellTargetPositions` is represented and wired at world startup (`WorldStatements::SEL_SPELL_TARGET_POSITION`, `SpellTargetPositionStoreLikeCpp`), while `LoadSpellRequired`, `LoadSpellLearnSpells`, `LoadSpellGroups`, `LoadSpellGroupStackRules`, `LoadSpellProcs` explicit SQL rows, `LoadSpellRanks` rank-chain representation, `LoadSpellAreas`, `LoadSpellThreats`, `LoadSpellPetAuras`, `LoadSpellEnchantProcData`, `LoadSpellLinked` and `LoadSpellTotemModel` have represented query/store coverage but are not wired into a live `SpellMgr` startup path yet. Implicit proc-entry generation from aura metadata and `LoadSpellInfoCustomAttributes` remain missing.
