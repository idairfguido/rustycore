...
| `SpellMgr::LoadSpellLearnSkills` | `SpellLearnSkill.dbc` (now DB2) → `mSpellLearnSkills` | `sSpellLearnSkillStore` |
...
| `SELECT spell, pet, aura, removeOnChangePet, damage FROM spell_pet_auras` | Owner aura → pet aura mapping | world | `LoadSpellPetAuras` |
...
- [ ] **#SPELLMGR.19** Implement `LoadSpellPetAuras` (`spell_pet_auras` SQL): `PetAura` per (spellId, eff); supports `petEntry==0` wildcard (M)
...
- **`LoadSpellPetAuras` allows `petEntry == 0` as a wildcard** — the lookup `GetAura(petEntry)` first tries the exact petEntry, then falls back to entry 0. Don't reject 0 as invalid input.
