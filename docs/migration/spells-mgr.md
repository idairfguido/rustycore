...
| `SpellMgr::LoadSpellRanks` | `spell_ranks` SQL → `mSpellChains` (also patches `SpellInfo::ChainEntry`) | `WorldDatabase.Query` |
...
3. Several SQL `spell_*` table loaders are still absent or only represented. `LoadSpellTargetPositions` is represented and wired at world startup (`WorldStatements::SEL_SPELL_TARGET_POSITION`, `SpellTargetPositionStoreLikeCpp`), while `LoadSpellRequired`, `LoadSpellLearnSpells`, `LoadSpellGroups`, `LoadSpellGroupStackRules`, `LoadSpellProcs` explicit SQL rows, `LoadSpellThreats`, `LoadSpellPetAuras`, `LoadSpellEnchantProcData`, `LoadSpellLinked` and `LoadSpellTotemModel` have represented query/store coverage but are not wired into a live `SpellMgr` startup path yet. `LoadSpellRanks`, implicit proc-entry generation from aura metadata, `LoadSpellAreas`, and `LoadSpellInfoCustomAttributes` remain missing.
...
- [ ] Test: `LoadSpellRanks` produces `prev/next/first/last/rank` matching C++ for the Frostbolt rank chain (1→3→4→5→6→7)
