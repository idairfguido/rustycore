...
| `SpellMgr::LoadSpellInfoServerside` | SQL `serverside_spell` + `serverside_spell_effect` — **server-only** spells that don't exist client-side (not the same as `spell_dbc` overlay) | `WorldDatabase.Query`, `mServersideSpellNames` |
...
| `SpellMgr::LoadSpellInfoCustomAttributes` | Computes `AttributesCu` bits from analysis of effects/auras (e.g. `SPELL_ATTR0_CU_DIRECT_DAMAGE` if any `SPELL_EFFECT_SCHOOL_DAMAGE`) + reads `spell_custom_attr` SQL | `WorldDatabase.Query` |
...
3. Several SQL/DB2 spell loaders are still absent or only represented. `LoadSpellTargetPositions` is represented and wired at world startup (`WorldStatements::SEL_SPELL_TARGET_POSITION`, `SpellTargetPositionStoreLikeCpp`), while `LoadSpellRequired`, `LoadSpellLearnSpells`, `LoadSpellGroups`, `LoadSpellGroupStackRules`, `LoadSpellProcs` explicit SQL rows, `LoadSpellRanks` rank-chain representation, `LoadSpellThreats`, `LoadSpellPetAuras`, `LoadSpellEnchantProcData`, `LoadSpellLinked` and `LoadSpellTotemModel` have represented query/store coverage but are not wired into a live `SpellMgr` startup path yet. Implicit proc-entry generation from aura metadata, `LoadSpellAreas`, and `LoadSpellInfoCustomAttributes` remain missing.
...
- [ ] **#SPELLMGR.4** Implement `LoadSpellInfoServerside` (`serverside_spell` + `serverside_spell_effect` SQL overlay; refuses to overlay an existing real DB2 spell) (H)
...
- [ ] Test: `LoadSpellAreas` populates all 5 indices (primary + quest-start + quest-end + aura + area) for a row with both a quest and an area trigger
