...
- [ ] **#SPELLMGR.19** Implement `LoadSpellPetAuras` (`spell_pet_auras` SQL): query + represented store/loader exist (`WorldStatements::SEL_SPELL_PET_AURAS`, `SpellPetAuraStoreLikeCpp`), including C++ key `(spell << 8) + eff`, dummy-effect validation, `petEntry==0` wildcard, and duplicate-key `AddAura` semantics; live startup wiring against the authoritative `SpellInfo` cache is still pending (M)
