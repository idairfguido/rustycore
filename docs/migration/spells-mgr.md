...
| `SpellMgr::LoadSpellInfoServerside` | SQL `serverside_spell` + `serverside_spell_effect` — **server-only** spells that don't exist client-side (not the same as `spell_dbc` overlay). Effect-row staging is represented; full server-only `SpellInfo` construction is pending. | `WorldDatabase.Query`, `mServersideSpellNames` |
...
| `SpellMgr::LoadPetLevelupSpellMap` | DB2 `CreatureFamily` + `SkillLineAbility` join → `mPetLevelupSpellMap` | `sCreatureFamilyStore`, `sSkillLineAbilityStore` |
| `SpellMgr::LoadPetDefaultSpells` | DB2 `CreatureTemplate` + cached → `mPetDefaultSpellsMap` | DB2 + cached creature templates |
...
| `SpellMgr::LoadPetFamilySpellsStore` | DB2 → static `sPetFamilySpellsStore` | `sCreatureFamilyStore` |
...
3. Several SQL/DB2 spell loaders are still absent or only represented. `LoadSpellTargetPositions` is represented and wired at world startup (`WorldStatements::SEL_SPELL_TARGET_POSITION`, `SpellTargetPositionStoreLikeCpp`), while `LoadSpellRequired`, `LoadSpellLearnSpells`, `LoadSpellGroups`, `LoadSpellGroupStackRules`, `LoadSpellProcs` explicit SQL rows, `LoadSpellRanks` rank-chain representation, `LoadSpellAreas`, `LoadSpellInfoCustomAttributes` SQL overrides, `LoadSpellThreats`, `LoadSpellPetAuras`, `LoadSpellEnchantProcData`, `LoadSpellLinked`, `LoadSpellTotemModel`, and `LoadSpellInfoServerside` effect-row staging have represented query/store coverage but are not wired into a live `SpellMgr` startup path yet. Implicit proc-entry generation from aura metadata, the full `serverside_spell` 81-column `SpellInfo` construction, and the derived `AttributesCu` computation pass remain missing.
4. The remaining `LoadSpellInfoServerside` SQL loader work: composing `serverside_spell` rows into server-only `SpellInfo` instances and preserving `mServersideSpellNames`.
...
- [ ] **#SPELLMGR.4** Implement `LoadSpellInfoServerside` (`serverside_spell` + `serverside_spell_effect` SQL overlay; refuses to overlay an existing real DB2 spell): represented queries exist (`WorldStatements::SEL_SERVERSIDE_SPELL_EFFECT`, `SEL_SERVERSIDE_SPELL`) and effect-row staging exists (`ServersideSpellEffectStoreLikeCpp`), including C++ regular-spell rejection, difficulty/effect/aura/target validation, grouping by `(spellId, difficulty)`, and C++-faithful invalid-radius warning without skip. Still pending: `serverside_spell` 81-column row representation, `mServersideSpellNames`, live `mSpellInfoMap.emplace(...)`, and startup wiring. (H)
...
- [ ] **#SPELLMGR.6** Implement `LoadSpellRanks` (DB2 `SkillLineAbilityEntry::SupercedesSpell`): represented `wow-data` store/accessors exist (`SpellChainStoreLikeCpp`) and build `SpellChainNode { prev, next, first, last, rank }` from skill-line supercession links, skipping missing spell infos like C++. Still pending: live `SpellMgr` startup wiring and patching real `SpellInfo::chain_entry` for every difficulty variant. (M)
...
- `LoadSpellRanks` / rank chains — represented Rust store/accessors exist in `wow-data` (`SpellChainStoreLikeCpp`) and are derived from `SkillLineAbilityEntry::SupercedesSpell` like current C++; live `SpellMgr` startup wiring and `SpellInfo::ChainEntry` patching are still absent.
...
- `spell_group` — represented query/store exists (`WorldStatements::SEL_SPELL_GROUP`, `SpellGroupStoreLikeCpp`) with recursive nested-group expansion, but startup wiring and stack-rule runtime consumption are still pending.
- `spell_group_stack_rules` — represented query/store exists (`WorldStatements::SEL_SPELL_GROUP_STACK_RULES`, `SpellGroupStackRuleStoreLikeCpp`) with C++ validation and same-effect aura inference, but it is not yet loaded into a live `SpellMgr` or consumed by the real aura stacking runtime. Cross-spell stacking semantics are therefore represented but not active in gameplay.
- `spell_proc` — represented explicit SQL-row loader exists (`WorldStatements::SEL_SPELL_PROC`, `SpellProcStoreLikeCpp`) with rank-chain expansion and primary validation, but implicit aura-generated proc entries, live `SpellMgr` wiring and the actual proc runtime are still absent.
...
- `serverside_spell` / `serverside_spell_effect` (the modern `spell_dbc` overlay) — represented Rust query coverage and `serverside_spell_effect` staging exist, but full `serverside_spell` row composition into server-only `SpellInfo`, `mServersideSpellNames`, and live startup wiring are still absent. Server-only spells (used by scripts to apply bookkeeping auras with no client visualization) are therefore not live yet.
