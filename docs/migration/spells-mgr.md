...
- [ ] **#SPELLMGR.4** Implement `LoadSpellInfoServerside` (`serverside_spell` + `serverside_spell_effect` SQL overlay; refuses to overlay an existing real DB2 spell): represented queries exist (`WorldStatements::SEL_SERVERSIDE_SPELL_EFFECT`, `SEL_SERVERSIDE_SPELL`) and represented stores exist (`ServersideSpellEffectStoreLikeCpp`, `ServersideSpellStoreLikeCpp`), including C++ regular-spell rejection, effect-row difficulty/effect/aura/target validation, grouping by `(spellId, difficulty)`, C++-faithful invalid-radius warning without skip, `serverside_spell` 81-column field preservation, `mServersideSpellNames` shape, and C++-faithful lack of main-row difficulty validation. World-server startup now loads the represented staging stores against live regular-spell/difficulty/radius existence and injects the serverside spell store into sessions. Still pending: live `mSpellInfoMap.emplace(...)` and authoritative `SpellInfo` ownership/insertion. (H)
...
- [ ] **#SPELLMGR.19** Implement `LoadSpellPetAuras` (`spell_pet_auras` SQL): query + represented `wow-data` store/loader exist (`WorldStatements::SEL_SPELL_PET_AURAS`, `SpellPetAuraStoreLikeCpp`), including C++ key `(spell << 8) + eff`, dummy-effect validation, `petEntry==0` wildcard, duplicate-key `AddAura` semantics, and source-effect `CalcValue()` via represented no-caster die-roll semantics. World-server startup now builds the store and injects it into sessions with `SpellMgr::GetPetAura` lookup semantics. Owner→pet aura runtime consumption is still pending. (M)
...
- [ ] **#SPELLMGR.21** Implement `LoadSpellLinked` (`spell_linked_spell` SQL): query + represented `wow-data` store/loader exist (`WorldStatements::SEL_SPELL_LINKED`, `SpellLinkedStoreLikeCpp`), including C++ trigger/effect existence checks, signed effect preservation, negative-trigger coercion to REMOVE, invalid-type/self-loop skips, vector push order, and same-base-point warning as non-fatal. World-server startup now builds the store and injects it into sessions with `SpellMgr::GetSpellLinked`-style lookup semantics. Cast/hit/aura/remove runtime consumption is still pending. (M)
- [ ] **#SPELLMGR.22** Implement `LoadPetLevelupSpellMap` (DB2 join `CreatureFamily` × `SkillLineAbility`): represented `PetLevelupSpellStoreLikeCpp` exists in `wow-data`, keyed by petFamily and preserving C++ `PetLevelupSpellSet` multimap order by `SpellLevel`, with C++ filters for `AcquireMethod == LEARNED_ON_SKILL_LEARN`, missing `SpellInfo`, and `SpellLevel == 0`. World-server startup now loads `CreatureFamily.db2`/`SpellLevels.db2`, builds the store against `SkillStore` + live `SpellStore`, and injects it into sessions with `SpellMgr::GetPetLevelupSpellList` lookup semantics. Pet learning/runtime consumption is still pending. (M)
- [ ] **#SPELLMGR.23** Implement `LoadPetDefaultSpells`: represented `PetDefaultSpellStoreLikeCpp` exists in `wow-data`, scanning `DIFFICULTY_NONE` summon/summon-pet effects, looking up cached creature templates, copying C++ `PetDefaultSpellsEntry { spellid: [u32; 4] }`, removing spells already covered by `mPetLevelupSpellMap`, and skipping empty results. World-server startup now builds the store from live `SpellStore`, creature templates, and the live pet-levelup store, and injects it into sessions with `SpellMgr::GetPetDefaultSpellsEntry` lookup semantics. Pet default spell learning/runtime consumption is still pending. (M)
- [ ] **#SPELLMGR.24** Implement `LoadSpellAreas` (`spell_area` SQL): query + represented `wow-data` store/loader exist (`WorldStatements::SEL_SPELL_AREA`, `SpellAreaStoreLikeCpp`), including C++ row validation, duplicate-similar-requirement skip, area/quest/aura/race/gender checks, autocast-aura-chain rejection, primary map and four secondary indices (`for_quest_start_or_end`, `for_quest_end`, `for_aura`, `for_area`). World-server startup now loads the store against live spell/area/quest existence and injects it into sessions with represented index accessors. Still pending: `SpellInfo` autocast `SPELL_ATTR0_NO_AURA_CANCEL` mutation, `SpellArea::IsFitToRequirements` runtime with Player/BG/Battlefield checks, and aura apply/remove integration. (H)
- [ ] **#SPELLMGR.25** Implement `LoadPetFamilySpellsStore`: represented `PetFamilySpellStoreLikeCpp` exists in `wow-data`, preserving C++ `std::map<uint32, std::set<uint32>>` shape and filters (`GetSpellInfo(..., DIFFICULTY_NONE)`, `SpellLevels.DifficultyID == 0 && SpellLevel != 0` skip, `SpellInfo::IsPassive`, creature-family skill-line match, `AcquireMethod == LEARNED_ON_SKILL_LEARN`). World-server startup now builds it from live `SkillStore`, `CreatureFamily.db2`, `SpellLevels.db2`, `SpellStore`, and `SpellMisc.db2` passive attributes, then injects it into sessions with represented family-spell lookup semantics. Pet passive learning/runtime consumption is still pending. (L)
- [ ] **#SPELLMGR.26** Implement `LoadSpellTotemModel` (`spell_totem_model` SQL): query + represented `wow-data` store/loader exist (`WorldStatements::SEL_SPELL_TOTEM_MODEL`, `SpellTotemModelStoreLikeCpp`), including C++ spell/race/display validation, duplicate overwrite semantics, and `GetModelForTotem` missing→0. World-server startup now loads `ChrRaces.db2`, builds the totem model store against live spell/race/display stores, and injects it into sessions. Totem summon display consumption is still pending. (L)
- [ ] **#SPELLMGR.27** Implement `LoadSpellInfoCustomAttributes`: query + represented SQL override store exists (`WorldStatements::SEL_SPELL_CUSTOM_ATTR`, `SpellCustomAttributeStoreLikeCpp`), including C++ missing-spell skip, per-difficulty variant application where variants are available, OR accumulation, and `SPELL_ATTR0_CU_SHARE_DAMAGE` requiring `SPELL_EFFECT_SCHOOL_DAMAGE`. World-server startup now loads the SQL override store against the current live `SpellStore` variant set and injects it into sessions with represented lookup semantics. Still pending: authoritative mutable `SpellInfo::AttributesCu`, full multi-difficulty `SpellInfo` ownership, and the large derived `AttributesCu` computation pass from effect/aura analysis, enchant procs, talents, visuals, liquids and post-pass binary/crit fixes. (H)
...
- `spell learn skills` — represented Rust store exists in `wow-data`, is built during world-server startup from represented `SpellInfo` effects, and is injected into sessions with C++ `GetSpellLearnSkill` lookup semantics. Skill learn/unlearn runtime consumption is still absent.
- `spell_learn_spell` — represented query/store exists (`WorldStatements::SEL_SPELL_LEARN_SPELL`, `SpellLearnSpellStoreLikeCpp`), is loaded during world-server startup from SQL plus `SpellLearnSpell.db2` and represented `SpellInfo` effects, and is injected into sessions with C++ multimap lookup semantics. The `Player::add_spell` learn/unlearn cascade runtime is still absent.
...
- `spell_pet_auras` — represented Rust query/store exists in `wow-data`, is loaded during world-server startup, and is injected into sessions with C++ `GetPetAura(spell, effect)` lookup semantics. Owner→pet aura runtime consumption is still absent, so owner→pet aura inheritance (Beast Mastery hunter, Demonology warlock) is loaded but not live in gameplay yet.
- `pet levelup spells` — represented Rust store exists in `wow-data`, is built during world-server startup from `CreatureFamily.db2` × `SkillLineAbility.db2` using live spell existence and `SpellLevels.db2` for C++ `SpellInfo::SpellLevel`, and is injected into sessions with C++ `GetPetLevelupSpellList` lookup semantics. Pet spell learning/runtime consumption is still absent.
- `pet default spells` — represented Rust store exists in `wow-data`, is built during world-server startup from summon/summon-pet spell effects and loaded creature templates, removes spells already covered by pet levelup spells, and is injected into sessions with C++ `GetPetDefaultSpellsEntry` lookup semantics. Pet default spell learning/runtime consumption is still absent.
- `pet family spells` — represented Rust store exists in `wow-data`, is built during world-server startup from `SkillLineAbility.db2`, `CreatureFamily.db2`, `SpellLevels.db2`, and `SpellMisc.db2` passive attributes, and is injected into sessions with represented family passive spell lookup semantics. Pet family passive learning/runtime consumption is still absent.
...
- `spell_linked_spell` — represented Rust query/store exists in `wow-data`, is loaded during world-server startup, and is injected into sessions with C++ `GetSpellLinked(type, spellId)` lookup semantics. Cast/hit/aura/remove runtime consumption is still absent, so Spell-A-triggers-Spell-B chains (used heavily by boss scripts) are loaded but not live in gameplay yet.
- `spell_area` — represented Rust query/store exists (`WorldStatements::SEL_SPELL_AREA`, `SpellAreaStoreLikeCpp`) with validation and indices, is loaded during world-server startup against live spell/area/quest existence, and is injected into sessions with represented primary/secondary lookup semantics. Zone-conditional auras (sanctum buffs, capital city resting buffs, racial flight masters) remain inactive until fit checks and aura apply/remove runtime are wired.
- `spell_totem_model` — represented Rust query/store exists in `wow-data`, is loaded during world-server startup against live spell/race/display stores, and is injected into sessions with C++ `GetModelForTotem` lookup semantics. Totem summon display consumption is still absent, so race-specific totem display (Tauren vs Orc shaman totems) is loaded but not live in gameplay yet.
- `spell_custom_attr` — represented Rust query/store exists (`WorldStatements::SEL_SPELL_CUSTOM_ATTR`, `SpellCustomAttributeStoreLikeCpp`) for SQL-driven `AttributesCu` overrides, is loaded during world-server startup against the current live `SpellStore` variant set, and is injected into sessions with represented lookup semantics. Authoritative mutable `SpellInfo::AttributesCu`, full multi-difficulty `SpellInfo` ownership, and derived `AttributesCu` computation remain absent.
- `serverside_spell` / `serverside_spell_effect` (the modern `spell_dbc` overlay) — represented Rust query/store coverage exists for row/effect staging and `mServersideSpellNames` shape, is loaded during world-server startup against live regular-spell/difficulty/radius existence, and is injected into sessions with represented lookup semantics. Authoritative `SpellInfo` insertion is still absent, so server-only spells (used by scripts to apply bookkeeping auras with no client visualization) are staged but not live yet.
