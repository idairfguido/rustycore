...
- `crates/wow-database/src/statements/character.rs` — 1826 lines — `CharStatements` enum (129 named variants + generated C++ SQL parity harness — partial as a named registry)
...
- **Character prepared statements**: 129 named Rust variants vs TC's 523, plus a generated parity harness that extracts all 523 C++ `PrepareStatement(CHAR_...)` SQL strings for coverage tests. The named registry is still partial; recent work added the initial pool-quest / ban / mail-list cluster and corrected `SEL_CHECK_NAME` + `SEL_SUM_CHARS` to C++ exact SQL. `SEL_ENUM` remains intentionally deferred because switching it to the full C++ column shape requires updating `handle_enum_characters` column indexes in the same slice.
...
- [ ] **#DB.11** Port the remaining character prepared statements as named Rust variants (cross-ref `INVENTORY.md`, `characters.md`). Current state: 129 named variants / 523 C++ statements; generated tests can extract all 523 exact SQL strings, but callers still need named variants per domain. (XL — split per-domain: inventory, achievements, mails, guilds, BG/arena state, etc.)
