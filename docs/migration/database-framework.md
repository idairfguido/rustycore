...
- `crates/wow-database/src/statements/character.rs` — 4559 lines — `CharStatements` enum (398 named variants + generated C++ SQL parity harness — partial as a named registry)
...
- [ ] **#DB.11** Port the remaining character prepared statements as named Rust variants (cross-ref `INVENTORY.md`, `characters.md`). Current state: 398 named variants / 523 C++ statements; generated tests can extract all 523 exact SQL strings, but callers still need named variants per domain. (XL — split per-domain: inventory, achievements, mails, guilds, BG/arena state, etc.)
