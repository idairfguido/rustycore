...
- `crates/wow-database/src/statements/character.rs` — 6118 lines — `CharStatements` enum (556 named variants, including compatibility aliases + generated C++ SQL parity harness — partial as a named registry)
...
- [ ] **#DB.11** Port the remaining character prepared statements as named Rust variants (cross-ref `INVENTORY.md`, `characters.md`). Current state: 556 named variants including compatibility aliases; exact C++ enum-name gap is down to 8 names: 4 BlackMarket future-scope statements and 4 `SelectItemInstanceContent` active-loader aliases. Generated tests can extract all 523 exact SQL strings, but the remaining active-loader aliases still need index review before claiming canonical named closure. (XL — split per-domain: inventory, achievements, mails, guilds, BG/arena state, etc.)
