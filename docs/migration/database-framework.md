...
- [x] Test: `Database::<LoginStatements>::open_with_pool_size(uri, 1)` succeeds; spawn 100 concurrent `query` calls against `SEL_REALMLIST` and verify all complete (queue serialization). Covered by the ignored live MariaDB harness under `#DB.16a` with a minimal `realmlist` table matching C++ `LOGIN_SEL_REALMLIST`.
...
- [x] Test: `DbUpdater::update` detects a renamed file (same hash, different filename) and updates the `name` column instead of re-applying. Pure C++ decision coverage is present, including the "old file still exists => treat as copy/new file" guard; the ignored live MariaDB harness under `#DB.16a` now verifies the DB row rename using an update body that would fail if re-applied.
