...
- [ ] **#AI.41b** Refactor random AI/Map restante: cuando existan `CombatAI`/`SmartAI`/`PetAI` runtime reales, mover `SelectRandomContainerElement`, `RandomResize`, `rand32`, `urand` y `frand` a RNG propiedad del mapa/AI owner correspondiente, sin `thread_rng` ni defaults time-based en gameplay. (M)
