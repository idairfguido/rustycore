...
- [~] **#GLOB.57** `load_player_choices + _locale`. `PlayerChoiceStoreLikeCpp` now represents the core C++ `ObjectMgr::LoadPlayerChoices` pass over `playerchoice`, `playerchoice_response`, and base `playerchoice_response_reward`: base choice fields, response append order from `ORDER BY Index ASC`, `RewardQuestID` optional, reward scalar fields, C++ validation semantics for invalid Title/QuestPackage/SkillLine rows (zeroed fields, with the C++ Package/SkillLine diagnostic-id bug intentionally corrected in Rust logs), C++ lookup helpers (`GetPlayerChoice`, `GetResponse`, `GetResponseByIdentifier`), missing-choice/response skip logging, startup loading after required DB2 validation stores, and exact SQL statement coverage. Remaining: reward item/currency/faction/item-choice rows (`playerchoice_response_reward_*`), MawPower rows, `LoadPlayerChoicesLocale`, live `DisplayPlayerChoice` packet/handler/script reward flow.
