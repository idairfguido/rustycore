...
- [~] **#GLOB.57** `load_player_choices + _locale`. `PlayerChoiceStoreLikeCpp` now represents the core C++ `ObjectMgr::LoadPlayerChoices` pass over `playerchoice`, `playerchoice_response`, base `playerchoice_response_reward`, `playerchoice_response_reward_item`, and `playerchoice_response_reward_currency`: base choice fields, response append order from `ORDER BY Index ASC`, `RewardQuestID` optional, reward scalar fields, reward items with C++ `BonusListIDs` whitespace token parsing, reward currencies, C++ validation semantics for invalid Title/QuestPackage/SkillLine rows (zeroed fields, with the C++ Package/SkillLine diagnostic-id bug intentionally corrected in Rust logs), C++ item-template/currency validation and skip semantics, C++ lookup helpers (`GetPlayerChoice`, `GetResponse`, `GetResponseByIdentifier`), missing-choice/response/reward skip logging, startup loading after required DB2 validation stores, and exact SQL statement coverage. Remaining: reward faction/item-choice rows (`playerchoice_response_reward_faction`, `_item_choice`), MawPower rows, `LoadPlayerChoicesLocale`, live `DisplayPlayerChoice` packet/handler/script reward flow.
