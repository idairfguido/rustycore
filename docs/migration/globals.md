...
- [ ] **#GLOB.39** `load_area_trigger_teleports` + `_tavern` + `_scripts` + `_involvedrelation`. Teleport fallback plus represented `_tavern`/`_scripts`/quest-involved loaders exist in `wow-data/area_trigger.rs`; startup wiring waits for an authoritative `AreaTriggerStore` instead of the current teleport-only fallback.
