...
- [ ] **#GLOB.12** `load_script_names` + `ScriptNameContainer` (interner). Core interner exists in `wow-data`; a deterministic template-store builder covers currently loaded creature/gameobject template script names, but direct world-DB loader wiring for every C++ script-name source remains pending. (L)
