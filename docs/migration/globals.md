...
- [~] **#GLOB.57** `load_player_choices + _locale`. `PlayerChoiceStoreLikeCpp` now represents the core C++ `ObjectMgr::LoadPlayerChoices` pass over `playerchoice` and `playerchoice_response`: base choice fields, response append order from `ORDER BY Index ASC`, `RewardQuestID` optional, C++ lookup helpers (`GetPlayerChoice`, `GetResponse`, `GetResponseByIdentifier`), missing-choice response skip logging, startup loading, and exact SQL statement coverage. Remaining: reward rows (`playerchoice_response_reward*`), MawPower rows, `LoadPlayerChoicesLocale`, live `DisplayPlayerChoice` packet/handler/script reward flow.
