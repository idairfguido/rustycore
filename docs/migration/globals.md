...
- [ ] **#GLOB.39** `load_area_trigger_teleports` + `_tavern` + `_scripts` + `_involvedrelation`. The legacy teleport fallback still feeds movement today, but represented C++-faithful `_teleport`/`_tavern`/`_scripts`/quest-involved loaders exist in `wow-data/area_trigger.rs`; startup wiring waits for an authoritative `AreaTriggerStore` instead of the current teleport-only fallback.
...
- [~] **#GLOB.42** `load_exploration_base_xp`. `ExplorationBaseXpStoreLikeCpp` mirrors C++ `ObjectMgr::LoadExplorationBaseXP` / `GetBaseXP` from world SQL `exploration_basexp` (`level`, `basexp`), including duplicate-level overwrite and missing-level `0` semantics. Remaining: startup wiring and exploration XP runtime consumption.
...
- [~] **#GLOB.44** `load_mail_level_rewards`. `MailLevelRewardStoreLikeCpp` mirrors C++ `ObjectMgr::LoadMailLevelRewards` (`level`, `raceMask`, `mailTemplateId`, `senderEntry`) with `MAX_LEVEL`, playable-race, mail-template and sender-creature validation plus `GetMailLevelReward(level, race)` first-match semantics. Remaining: startup wiring and level-up mail delivery runtime consumption.
...
- [~] **#GLOB.50** `load_game_tele`. `GameTeleStoreLikeCpp` mirrors C++ `ObjectMgr::LoadGameTele` / `GetGameTele` / `GetGameTeleExactName` over world DB `game_tele`, including map-coordinate validation, duplicate-ID overwrite, case-insensitive exact lookup, and partial-name fallback lookup; startup loads/logs it. Remaining: GM `.tele`/lookup command consumers, online `AddGameTele` / `DeleteGameTele` persistence, and DB parity tests.
- [~] **#GLOB.51** `load_gossip_menu` + `load_gossip_menu_items` + `_locales` + `_addon`. `GossipStore` now loads represented C++ ObjectMgr rows for `gossip_menu`, full `gossip_menu_option`, `gossip_menu_option_locale` (skipping invalid/enUS locales), and `gossip_menu_addon`, with multimap-style menu/item bounds plus ConditionMgr attachment seams. Remaining: C++ cross-store validations (`npc_text`, broadcast text, language, POI, GossipNPCOption, spell, faction/friendship reputation), replacing live per-session DB gossip queries with the global store, packet/localization parity, and runtime option actions.
- [~] **#GLOB.52** `load_vendors` + `add_vendor_item` (online editor support) + persistence. `NpcVendorStoreLikeCpp` now represents C++ `ObjectMgr::LoadVendors` / `LoadReferenceVendor` / `GetNpcVendorItemList` / `VendorItemData` / in-memory add/remove, including ordered `npc_vendor` load, negative reference-vendor expansion, bonus-list parsing, maxcount/incrtime local guards, currency maxcount guard, and duplicate `(item, ExtendedCost, type)` filtering; startup loads/logs it. Remaining: C++ cross-store validation against creature templates/npcflags, item templates/currencies, player conditions, extended costs; live vendor list/buy handlers still query DB per-session and must be moved to the global store; online `AddVendorItem`/`RemoveVendorItem` persistence and game-event live cache merge need final integration.
- [~] **#GLOB.53** `load_trainers` + `load_creature_trainers`. `TrainerStoreLikeCpp` now represents C++ `ObjectMgr::LoadTrainers` / `LoadCreatureTrainers` / `GetTrainer` / `GetCreatureTrainerForGossipOption`, including startup `trainer_spell`, `trainer`, `trainer_locale` (skipping invalid/enUS locales), and `creature_trainer` loading plus trainer-spell/locales/missing-trainer diagnostics. Remaining: C++ cross-store validation against SpellMgr, SkillLine, CreatureTemplate, and GossipMenu option data; live trainer list/buy handlers still query DB per-session and need to consume this global store; exact `Trainer::SendSpells` / `TeachSpell` behavior must be ported, including C++ spell-state numeric values (`Known=0`, `Available=1`, `Unavailable=2`), reputation discount, class/race fit, skill/ability/rank gates, money, profession-slot checks, learned spell effects, and purchase-side condition revalidation.
- [~] **#GLOB.54** `load_phases` + `unload_phase_conditions` + `load_terrain_swap_defaults` + `load_terrain_world_maps` + `load_area_phases` + `load_phase_names`. Existing Rust already represents `Phase.db2` / `PhaseXPhaseGroup.db2`, `PhaseInfoStore::LoadAreaPhases`, condition attachment, `TerrainSwapStore` for `terrain_worldmap` + `terrain_swap_defaults`, and runtime DB-spawn phase/terrain-swap application seams. This slice adds `PhaseNameStoreLikeCpp` for C++ `ObjectMgr::LoadPhaseNames` / `GetPhaseName` with the exact `"Unknown Name"` fallback and startup loading. Remaining: full `UnloadPhaseConditions` lifecycle/reload path, exact C++ diagnostic reporting for invalid terrain/area/phase rows, wiring `PhaseNameStoreLikeCpp` into GM/debug phase printing, and full runtime `PhasingHandler` area/condition/aura propagation.
- [~] **#GLOB.55** `load_faction_change_*` (achievements, items, quests, reputations, spells, titles). `FactionChangeStoreLikeCpp` now represents the SQL-backed C++ `ObjectMgr::LoadFactionChangeAchievements` / `LoadFactionChangeQuests` / `LoadFactionChangeReputations` / `LoadFactionChangeSpells` / `LoadFactionChangeTitles`, including exact `SELECT alliance_id, horde_id ...` statements, DB2/Quest/Spell validation, skipped invalid pair diagnostics, duplicate-alliance overwrite semantics, and C++ row-count behavior where invalid rows are still counted. It also represents `LoadFactionChangeItems` by deriving Alliance->Horde / Horde->Alliance maps from `ItemSparse::FactionRelated` and `Flags2` (`ITEM_FLAG2_FACTION_*`) like C++ `ItemTemplate::GetOtherFactionItemId()`. Startup loads/logs the represented mappings. Remaining: live character race/faction-change handlers are still missing.
- [~] **#GLOB.56** `load_scene_templates`. `SceneTemplateStoreLikeCpp` now represents C++ `ObjectMgr::LoadSceneTemplates` over `scene_template`, including `SceneId`, playback flags, scene package ID, encrypted flag, `GetScriptId(ScriptName)` interning, duplicate SceneId overwrite semantics, startup loading/logging, and `GetSceneTemplate` lookup. Note: canonical C++ appears to have a logging bug here (`count` is declared but never incremented), so Rust records both real rows seen and the C++ log-count bug value. Remaining: wire into live `SceneMgr` / spell scene-object creation / SmartAI scene script runtime.
- [~] **#GLOB.57** `load_player_choices + _locale`. `PlayerChoiceStoreLikeCpp` now represents the core C++ `ObjectMgr::LoadPlayerChoices` and `LoadPlayerChoicesLocale` passes over `playerchoice`, `playerchoice_response`, base `playerchoice_response_reward`, `playerchoice_response_reward_item`, `playerchoice_response_reward_currency`, `playerchoice_response_reward_faction`, `playerchoice_response_reward_item_choice`, `playerchoice_response_maw_power`, `playerchoice_locale`, and `playerchoice_response_locale`: base choice fields, response append order from `ORDER BY Index ASC`, `RewardQuestID` optional, reward scalar fields, reward items and item choices with C++ `BonusListIDs` whitespace token parsing, reward currencies, reward factions, optional MawPower fields, non-enUS choice/response locale strings, C++ validation semantics for invalid Title/QuestPackage/SkillLine rows (zeroed fields, with the C++ Package/SkillLine diagnostic-id bug intentionally corrected in Rust logs), C++ item-template/currency/faction validation and skip semantics, C++ locale reload/skip behavior, C++ lookup helpers (`GetPlayerChoice`, `GetPlayerChoiceLocale`, `GetResponse`, `GetResponseByIdentifier`), missing-choice/response/reward skip logging, startup loading after required DB2 validation stores, and exact SQL statement coverage. Remaining: live `DisplayPlayerChoice` packet/handler/script reward flow.
- [~] **#GLOB.58** `load_jump_charge_params`. `JumpChargeParamsStoreLikeCpp` now mirrors C++ `ObjectMgr::LoadJumpChargeParams` over `jump_charge_params`, including exact SQL, duplicate-id overwrite, `SPEED_CHARGE`/`Movement::gravity` fallback for invalid non-positive values, optional `SpellVisual`/`Curve` validation, startup loading/logging after the required DB2 stores, and `GetJumpChargeParams` lookup semantics. Note: canonical C++ has two diagnostic-only typos here (invalid progress curve logs the `spellVisualId` column, and the final load log says "Player Choice locale strings"); Rust keeps the C++ data behavior but logs the real jump-charge diagnostics. Remaining: wire the store into live `Spell::EffectJumpCharge` / movement launch packet construction.
- [~] **#GLOB.59** `load_creature_quest_items` + `load_game_object_quest_items`. `CreatureQuestItemStoreLikeCpp` and `GameObjectQuestItemStoreLikeCpp` now mirror C++ `ObjectMgr::LoadCreatureQuestItems` / `LoadGameObjectQuestItems` over `creature_questitem` and `gameobject_questitem`, including exact global SQL, `ORDER BY Idx ASC` append order, creature/gameobject template validation, item validation against `ItemSparse`, C++ skip diagnostics, gameobject lookup by entry, and creature lookup by `(entry, Difficulty)` with `Difficulty.db2` fallback semantics. Startup loads/logs both represented stores after the required template/item DB2 stores. Remaining: replace live per-query `gameobject_questitem` DB lookup and creature/gameobject query-response tooltip paths with the global stores.

### Phase E — DataStore companions

- [~] **#GLOB.60** Port `AreaTriggerDataStore` (template + create_properties + actions/spline/polygon). 60A represents the first C++ `AreaTriggerDataStore::LoadAreaTriggerTemplates` stage for `areatrigger_template_actions` + `areatrigger_template`: exact SQL, action type / target type validation, teleport-action `WorldSafeLoc` validation, template flags, actions moved into matching templates, startup loading/logging, and `GetAreaTriggerTemplate`/ConditionMgr key lookup semantics through `AreaTriggerTemplateStore`. 60B stages `areatrigger_create_properties_polygon_vertex` and `areatrigger_create_properties_spline_point` with exact SQL/order, base polygon vertices, optional target vertices, spline point buckets, and partial target validation; note the C++ error log appears to print `IsCustom` where it means `Idx`, while Rust records the real `Idx`. 60C loads `areatrigger_create_properties` with exact SQL, template/shape validation, Curve.db2 validation/reset, polygon height normalization, target-count cleanup, spline attachment, and `ScriptName` interning. 60D attaches `areatrigger_create_properties_orbit` with exact SQL, invalid create-property reference skip, and non-finite float reset. Remaining: static `areatrigger` spawns, map/cell index, and live AreaTrigger runtime consumption.
