...
| L1 | shared/Realm | `src/server/shared/Realm/` | [shared-realm.md](shared-realm.md) | ⚠️ (~77%; BNet RealmHandle packing/storage/cfg swap, typed flags/types, hostname resolution, normalized names, build-version lookup, `JamJSONRealmEntry` and strong RealmHandle fixed; golden/e2e pending) | ⚠️ |
