...
- **Profanity / spam filter** — absent. `CMSG_CHAT_REPORT_FILTERED` is represented as the current C++ no-op/debug stub, but no actual spam-reporting backend exists. `GM_SILENCE_AURA=1852` is now represented for normal chat, chat-emote, AFK/DND, and whisper-to-non-GM gates, including the English C++ `LANG_GM_SILENCE` `SMSG_PRINT_NOTIFICATION`; DB-backed localization remains pending.
