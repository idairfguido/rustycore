...
- No `ChatFlood` rate-limit / `SMSG_CHAT_RESTRICTED` behavior yet. Persisted account `mutetime` and `GM_SILENCE_AURA` are represented, but a scripted client can still flood once allowed to speak.
...
- [x] **#CHAT.14b2n** Port C++ persisted account `mutetime` / `CanSpeak` chat gate — `world-server` preserves auth `account.mutetime`, sessions carry `m_muteTime`-like state, normal chat/whisper/chat-emote/text-emote/AFK/DND reject with represented English `LANG_WAIT_BEFORE_SPEAKING` `SMSG_PRINT_NOTIFICATION`, and addon chat is silently rejected like C++. Full `.mute/.unmute`, DB expiry update, DB-backed localization, and flood-throttle `SMSG_CHAT_RESTRICTED` remain pending. Complejidad: **S**
