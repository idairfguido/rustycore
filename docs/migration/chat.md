...
- **Addon messages** — `CMSG_CHAT_ADDON_MESSAGE` now honors the config-backed `AddonChannel` gate and routes Party/Raid/InstanceChat addon payloads through group membership and receiver-side addon-prefix filtering. Guild/Officer/Channel addon routing and targeted/whisper addon packets remain absent.
