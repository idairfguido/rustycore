...
- Say/Yell/chat-emote now mirror C++ `Player::IsAlive()` cheating gates and config-backed `ChatLevelReq.Say/Yell/Emote`, including represented `SMSG_PRINT_NOTIFICATION` `LANG_SAY_REQ` text on rejection; whisper applies config-backed `ChatLevelReq.Whisper` with the C++ GM sender exemption and represented `LANG_WHISPER_REQ` notification. `ChatLevelReq.Channel` is loaded into `SessionResources` but remains dormant until `ChannelMgr` exists.
...
- **Profanity / spam filter** — absent. `CMSG_CHAT_REPORT_FILTERED` is represented as the current C++ no-op/debug stub, but no actual spam-reporting backend exists. `GM_SILENCE_AURA=1852` is represented for normal chat, chat-emote, AFK/DND, and whisper-to-non-GM gates, including the English C++ `LANG_GM_SILENCE` `SMSG_PRINT_NOTIFICATION`; persisted account `mutetime` now feeds `WorldSession::CanSpeak`-like gates for normal chat, whisper, chat-emote, text-emote, AFK/DND, and generic addon rejection. Config-backed `ChatFlood.*` now updates `m_muteTime`-like state for regular chat and generic/targeted addon chat; the dedicated addon-whisper opcode intentionally follows C++ and does not call `CanSpeak`/`UpdateSpeakTime`. DB-backed localization, RBAC-exact spam-check exemption, DB persistence of flood mutes, and non-flood `SMSG_CHAT_RESTRICTED` behavior remain pending.
...
- `ChatFlood` is represented as C++ `UpdateSpeakTime` session counters, but RBAC-exact spam-check exemption and DB persistence for newly assigned mutes remain pending. `SMSG_CHAT_RESTRICTED` behavior remains absent for non-flood restrictions.
...
- [x] **#CHAT.14b2e** Port C++ Say/Yell/Emote level gates — config-backed `ChatLevelReq.Say/Yell/Emote` rejects represented senders below the configured level before delivery and sends represented English `LANG_SAY_REQ` via `SMSG_PRINT_NOTIFICATION`. Complejidad: **S**
- [x] **#CHAT.14b2i** Port C++ Whisper level gate — config-backed `ChatLevelReq.Whisper` rejects non-GM represented senders below the configured level before delivery and sends represented English `LANG_WHISPER_REQ` via `SMSG_PRINT_NOTIFICATION`; GM sender exemption is represented. Complejidad: **S**
...
- [x] **#CHAT.14b2m** Port represented C++ `LANG_GM_SILENCE` notification — normal chat, chat-emote, AFK/DND, and whisper-to-non-GM now send `SMSG_PRINT_NOTIFICATION` with English `LANG_GM_SILENCE` text before returning; DB-backed localization remains in `#CHAT.14b2b`. Complejidad: **S**
- [x] **#CHAT.14b2n** Port C++ persisted account `mutetime` / `CanSpeak` chat gate — `world-server` preserves auth `account.mutetime`, sessions carry `m_muteTime`-like state, normal chat/whisper/chat-emote/text-emote/AFK/DND reject with represented English `LANG_WAIT_BEFORE_SPEAKING` `SMSG_PRINT_NOTIFICATION`, and generic addon chat is silently rejected like C++. Full `.mute/.unmute`, DB expiry update, DB-backed localization, and non-flood `SMSG_CHAT_RESTRICTED` call sites remain pending. Complejidad: **S**
- [x] **#CHAT.14b2o** Port C++ `Player::UpdateSpeakTime` flood counters — `ChatFlood.MessageCount/Delay`, `ChatFlood.AddonMessageCount/Delay`, and `ChatFlood.MuteTime` are read from world config, sessions track regular/addon throttle windows, and exceeding the configured count updates `m_muteTime`-like state for the next send attempt without blocking the triggering message, matching C++. Dedicated addon whisper remains unthrottled like the inspected C++ handler. Complejidad: **S**
- [ ] **#CHAT.14b2b** Complete remaining C++ AFK/DND/chat side effects: packet-parser hyperlink exception kick, DB-backed localized notification/default strings, guild away event, script hook, and battleground leave. Complejidad: **M**
...
- `ChatLevelReq.Say/Yell/Emote/Whisper` are config-backed and represented rejections send C++ English `LANG_SAY_REQ` / `LANG_WHISPER_REQ` notifications; `ChatLevelReq.Channel` is parsed into the session resource snapshot but not behaviorally consumed until the channel system is ported.
- `GM_SILENCE_AURA = 1852` is now checked via represented visible auras for normal chat, chat-emote, AFK/DND, and whisper-to-non-GM. These represented handlers send C++ `SMSG_PRINT_NOTIFICATION` with English `LANG_GM_SILENCE` text before returning; DB-backed localization and any path outside represented chat handlers remain pending.
