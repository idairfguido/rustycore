...
18. **No SpellMgr vivo** — `spell_required`, `spell_learn_spell`, `spell_threat` y `spell_target_position` ya tienen stores representados en `wow-data` (el último además está cableado en world startup), pero no existe todavía una carga viva de `SpellMgr`; `spell_proc` y `spell_chain` siguen sin loader completo.
