...
18. **No SpellMgr vivo** — `spell_required`, `spell_learn_spell` y `spell_threat` ya tienen stores representados en `wow-data`, pero no existe todavía una carga viva de `SpellMgr`; `spell_proc`, `spell_target_position` y `spell_chain` siguen sin loader completo.
