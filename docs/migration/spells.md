...
18. **No SpellMgr vivo** — `spell_required`, `spell_learn_spell`, `spell_threat`, `spell_target_position`, `spell_group`, `spell_group_stack_rules`, `spell_area`, `spell_custom_attr` SQL overrides, `LoadSpellRanks`/rank chains y la mitad explícita SQL de `spell_proc` ya tienen stores representados en `wow-data` (target position además está cableado en world startup), pero no existe todavía una carga viva de `SpellMgr`; `spell_proc` sigue incompleto por la generación implícita desde auras, las cadenas de rango aún no parchean `SpellInfo::ChainEntry` real, `spell_area` no se consume en el runtime de auras y falta la derivación completa de `AttributesCu`.
...
**SpellMgr / SpellInfo / SpellHistory.** No live `SpellMgr` startup exists yet. Several represented DB2/SQL stores now exist in `wow-data` (`spell_required`, `spell_learn_spell`, `spell_threat`, `spell_target_position`, `spell_group`, `spell_group_stack_rules`, `spell_area`, `spell_custom_attr` SQL overrides, explicit `spell_proc` rows, rank chains via `SkillLineAbilityEntry::SupercedesSpell`, etc.), but implicit proc-entry generation, derived `AttributesCu` computation, live chain-entry patching, live spell-area runtime consumption, and `spell_script_names` still lack live loader coverage. The minimal `wow_data::SpellInfo` referenced from the handler exposes only a subset of the ~80 fields C++ `SpellInfo` carries.
