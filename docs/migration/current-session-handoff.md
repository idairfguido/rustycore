...
- Latest completed slice: `#NEXT.R8.ENTITIES.417 — Map-owned GridUnloadEntityStore bridge` (review `APROBADO`; validation OK; committed locally at `current #417 HEAD`; no push/install/restart).
...
Overall core migration estimate after #417 map-owned GridUnloadEntityStore bridge: `~90.7%` (was `~90.5%` after #416; do not claim >95% until ownership/runtime gaps below are substantially closed).
