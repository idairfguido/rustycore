...
- Latest completed slice: `#NEXT.R8.ENTITIES.423 — Map bridge for Unit shared-vision SetWorldObject requests` (review `APROBADO`; validation OK; committed locally at `current #423 HEAD`; no push/install/restart).
...
- No push/install/restart performed for #383 through #422.
...
Overall core migration estimate after #423 Unit shared-vision request -> Map facade bridge: `~91.2%` (was `~91.15%` after #422; do not claim >95% until ownership/runtime gaps below are substantially closed).
...
Manual test point: no new client-facing manual milestone from #410/#421/#423. Fixed-level and variable-level supported DB-backed Creature records plus supported intrinsic GameObject records can flow through the live loaded-grid respawn seam into `MapObjectRecord`/AddToMap with map-owned GUID generation. This now represents DB-backed GameObject linked-trap pre-add, map-local removal, deferred remove-list drain, temporary Unit switch-list container movement, a map-owned `WorldObject::SetWorldObject` enqueue facade over canonical records only, and an explicit Unit shared-vision request -> Map facade bridge. It still does not support creature vehicle templates, full AddToWorld/ObjectAccessor/fanout/scripts/AI/zonescript/formation/dynamic-tree side effects, full GameObject runtime, model/collision, visibility distance overrides, full transport runtime, ObjectGridLoader rewrite, PoolMgr full live spawn work, AreaTrigger runtime, GO_JUST_DEACTIVATED runtime, full CleanupsBeforeDelete, dynamic tree/session fanout, live `Player::SetViewpoint` callsite wiring, broader live `WorldObject::SetWorldObject` callsites, install, restart, or push.
