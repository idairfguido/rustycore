...
- Latest completed/current slice: `#NEXT.R8.ENTITIES.426 — DynamicObject caster SetViewpoint map-owned seam` (review `APROBADO`; validation OK; committed locally at `current #426 HEAD`; no push/install/restart).
...
- No push/install/restart performed for #383 through #426.
...
Overall core migration estimate after #426 local DynamicObject caster-viewpoint map-owned seam: `~91.45%` (was `~91.35%` after #425; do not claim >95% until ownership/runtime gaps below are substantially closed).
...
Manual test point: no new client-facing manual milestone from #410/#421/#423/#425/#426. Fixed-level and variable-level supported DB-backed Creature records plus supported intrinsic GameObject records can flow through the live loaded-grid respawn seam into `MapObjectRecord`/AddToMap with map-owned GUID generation. This now represents DB-backed GameObject linked-trap pre-add, map-local removal, deferred remove-list drain, temporary Unit switch-list container movement, a map-owned `WorldObject::SetWorldObject` enqueue facade over canonical records only, an explicit Unit shared-vision request -> Map facade bridge, a bounded map-owned `Player::SetViewpoint` Unit-target shared-vision consumption seam over canonical typed Player plus Creature/Pet records, a committed `CMSG_FAR_SIGHT` parser/handler represented session `m_seer` seam that requests the existing visibility refresh, and a bounded map-owned DynamicObject caster-viewpoint caller helper over canonical typed Player plus typed DynamicObject records. It still does not support creature vehicle templates, full `Player::SetViewpoint`, full SetSeer fanout/grid notifier/ObjectAccessor visibility, real `UpdateVisibilityOf`/visibility fanout, aura/cinematic/dynamic-object SetViewpoint callsites, full AddToWorld/ObjectAccessor/fanout/scripts/AI/zonescript/formation/dynamic-tree side effects, full GameObject runtime, model/collision, visibility distance overrides, full transport runtime, ObjectGridLoader rewrite, PoolMgr full live spawn work, AreaTrigger runtime, GO_JUST_DEACTIVATED runtime, full CleanupsBeforeDelete, dynamic tree/session fanout, broader live `WorldObject::SetWorldObject` callsites, install, restart, or push.
