...
> **Status:** ⚠️ first portable slices started — `wow-movement::PathGenerator` now has C++ constants/flags, no-navmesh shortcut fallback, geometry helpers, path length and `ShortenPathUntilDist`; `wow-recastdetour` now vendors and compiles legacy Detour C++, has first `dtNavMesh` Rust binding (`alloc/free/init/getMaxTiles/addTile/removeTile`) with real success/error smoke coverage, `dtNavMeshQuery` allocation/init/free, `findNearestPoly`, `findPath`, `findStraightPath`, closest-point helpers, `getPolyHeight`, `moveAlongSurface`, `raycast`, C++-like `CreateFilter/UpdateFilter` filter helpers, straight/raycast `BuildPolyPath` Detour helpers, first straight `BuildPointPath` helper, `FixupCorridor`/`GetSteerTarget` helpers and a `dtQueryFilter` wrapper, and has `MMapDefines.h` constants/flags, `MmapTileHeader`, `.mmtile` blob reader, Detour constants, `dtNavMeshParams` parsing and a C++-like `MMapManager` map cache with tile load/unload and per-instance query allocation, but no full pathgen integration yet
...
**What's missing vs C++:**
- **Detour-backed `PathGenerator` behavior** — the owner-backed `CalculatePath`, previous-path reuse, `BuildPointPath` dispatch, offmesh `FindSmoothPath` handling, `NormalizePath`, destination-Z validation and final game/runtime wiring remain unported.
...
- **PathGenerator integration** — `dtNavMesh` allocation/init/free/addTile/removeTile, `dtNavMeshQuery` allocation/init/free, `findNearestPoly`, `findPath`, `findStraightPath`, closest-point helpers, `getPolyHeight`, `moveAlongSurface`, `raycast`, `dtQueryFilter` flags/area costs, C++ filter helper rules, straight/raycast `BuildPolyPath`, `FixupCorridor`, `GetSteerTarget` and the normal `FindSmoothPath` loop now have wrappers/coverage; previous-path optimization, offmesh smooth-path endpoint handling and owner/map wiring remain open.
...
- [ ] **#MOVE-PATH.15** Port `BuildPolyPath`: first straight helper now covers `GetPolyByLocation` low/high extents, missing-poly shortcut/NOPATH, far-from-poly end clamp, same-poly and fresh `findPath` path generation; raycast helper now covers empty-path NOPATH, missing-poly far flags, hit/end clamp via `getPolyHeight`/boundary and normal/incomplete flags. Still open: previous-path reuse/suffix optimization. (H)
- [ ] **#MOVE-PATH.16** Port `BuildPointPath`: straight `findStraightPath` helper path is represented, including single-point same-poly append, short/nopath fallback and force-destination branch; raycast path branch is represented separately because C++ returns two-point raycast paths directly from `BuildPolyPath`. Still open: dispatch to `FindSmoothPath` when `_useStraightPath=false`. (H)
- [ ] **#MOVE-PATH.17** Port `FindSmoothPath` + `GetSteerTarget` + `FixupCorridor` (the Detour smooth-path tutorial code, identical math). `FixupCorridor`, `GetSteerTarget` and the normal `FindSmoothPath` loop are ported/tested, including `closestPointOnPolyBoundary`, `moveAlongSurface`, corridor fixup, height adjustment, end-of-path snap and C++ loop-failure guard. Remaining work: offmesh connection endpoint handling via `dtNavMesh::getOffMeshConnectionPolyEndPoints`. (XL)
