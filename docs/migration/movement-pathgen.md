...
- First hand-rolled Detour C ABI exists for `dtNavMesh`, `dtNavMeshQuery` allocation/init/free, `dtNavMeshQuery::findNearestPoly`, `findPath`, `findStraightPath`, closest-point helpers, `getPolyHeight` and `dtQueryFilter` flags/area costs; remaining query methods are still pending.
...
- **`dtNavMeshQuery` methods / filter integration** — `dtNavMesh` allocation/init/free/addTile/removeTile, `dtNavMeshQuery` allocation/init/free, `findNearestPoly`, `findPath`, `findStraightPath`, closest-point helpers, `getPolyHeight` and `dtQueryFilter` flags/area costs now have wrappers and smoke coverage, but the remaining query methods (`moveAlongSurface`, `raycast`, etc.) and `PathGenerator::CreateFilter/UpdateFilter` integration are still missing.
...
- [ ] **#MOVE-PATH.3** Generate FFI bindings (`bindgen` or hand-rolled) for `dtNavMesh` (`init`, `addTile`, `removeTile`, `getTileAndPolyByRef`, `getMaxTiles`), `dtNavMeshQuery` (`init`, `findNearestPoly`, `findPath`, `findStraightPath`, `closestPointOnPoly`, `moveAlongSurface`, `getPolyHeight`, `raycast`), `dtQueryFilter`. Partial: `dtNavMesh` `alloc/free/init/getMaxTiles/addTile/removeTile`, `dtNavMeshQuery` `alloc/free/init/findNearestPoly/findPath/findStraightPath/closestPointOnPoly/closestPointOnPolyBoundary/getPolyHeight` and `dtQueryFilter` include/exclude/area-cost APIs are hand-rolled and tested for smoke/error/success paths with generated Detour tile data where applicable. (XL — split per class)
