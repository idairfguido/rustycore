...
- `crates/wow-world/src/map_manager.rs` now initializes the local Detour `MMapManager` worker with that `mapData`, matching C++ `MMapManager::InitializeThreadUnsafe(mapData)`, and has the C++ `TerrainInfo` grid-file existence slice: optional `maps/{mapid:04}.tilelist` (`MAPS` v10 + `std::bitset::to_string()` ordering), fallback `.map` header validation, child-terrain lookup and `PhaseShift.VisibleMapIds` terrain-map resolution.
...
- `wow-world` has a narrow C++ `TerrainInfo::DiscoverGridMapFiles`/`HasChildTerrainGridFile` port for pathgen phasing: it reads `.tilelist` first, preserves C++ bitset text ordering, falls back to validating each `.map` header, recursively attaches child terrain from `mapData`, and resolves `GetTerrainMapId` against `PhaseShift.VisibleMapIds`.
...
- **Phasing terrain map runtime wiring** — C++ `PathGenerator` calls `PhasingHandler::GetTerrainMapId(phaseShift, ownerMapId, terrain, x, y)` before `GetNavMeshQuery`. Rust now has `PhaseShift.VisibleMapIds`, root/child `mapData`, `TerrainInfo` grid-file existence and the resolver, but the world mmap worker/session path request still needs to pass the owner's real phase shift and root terrain index into the Detour query selection.
