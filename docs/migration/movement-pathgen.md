...
**What's missing vs C++:**
- **Detour-backed `PathGenerator` behavior** — the owner-backed `CalculatePath` and final game/runtime wiring remain unported; portable/Detour helpers now cover the main `BuildPolyPath`, `BuildPointPath`, `FindSmoothPath`, `NormalizePath` and destination-Z pieces.
...
- [ ] **#MOVE-PATH.21** Port `CalculatePath(destX, destY, destZ, force_dest)` top-level entry orchestrating all the above. A Detour-level orchestrator now centralizes WoW/Detour coordinate swaps and calls `BuildPolyPath`/`BuildPointPath`; still open: owner runtime gates (`DisableMgr`, `HaveTile`, ignore-pathfinding state, map/instance query lookup) and writing results into the game `PathGenerator` owner object. (H)
