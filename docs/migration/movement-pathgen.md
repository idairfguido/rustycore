...
- **PathGenerator integration** — `dtNavMesh` allocation/init/free/addTile/removeTile, `dtNavMeshQuery` allocation/init/free, `findNearestPoly`, `findPath`, `findStraightPath`, closest-point helpers, `getPolyHeight`, `moveAlongSurface`, `raycast`, `dtQueryFilter` flags/area costs, C++ filter helper rules, straight/raycast `BuildPolyPath`, `FixupCorridor`, `GetSteerTarget` and `FindSmoothPath` now have wrappers/coverage; previous-path optimization, offmesh fixture coverage and owner/map wiring remain open.
