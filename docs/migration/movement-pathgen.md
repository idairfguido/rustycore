...
- [ ] **#MOVE-PATH.21** Port `CalculatePath(destX, destY, destZ, force_dest)` top-level entry orchestrating all the above. A Detour-level orchestrator now centralizes WoW/Detour coordinate swaps and calls `BuildPolyPath`/`BuildPointPath`; `HaveTile` is represented through real `dtNavMesh::calcTileLoc/getTileAt`; `MMapData::calculate_path_for_instance_like_cpp` now uses the per-instance query cache and returns `None` when query/tiles are unavailable; `wow-movement::PathGenerator::apply_detour_path_like_cpp` can write the result into portable path state. Still open: owner runtime gates (`DisableMgr`, ignore-pathfinding state, map/instance query lookup), `PathType` conversion at the runtime boundary and calling the bridge from the game owner object. (H)
