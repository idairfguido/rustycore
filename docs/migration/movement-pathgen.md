...
- **PathGenerator integration** — `dtNavMesh` allocation/init/free/addTile/removeTile, `dtNavMeshQuery` allocation/init/free, `findNearestPoly`, `findPath`, `findStraightPath`, closest-point helpers, `getPolyHeight`, `moveAlongSurface`, `raycast` and `dtQueryFilter` flags/area costs now have wrappers and smoke coverage, but `PathGenerator::CreateFilter/UpdateFilter`, `BuildPolyPath`, `BuildPointPath` and `FindSmoothPath` are not wired to them yet.
