...
- No canonical quest-only drops in `wow-loot`; represented `CMSG_OPEN_ITEM` item-loot bridge handles `QuestRequired`, `Quest::ItemDrop`, `ItemSparse.StartQuestID`, `QuestLogItemId` and `ITEM_FLAGS_CU_IGNORE_QUEST_STATUS` (#NEXT.R8.ENTITIES.189/#NEXT.R8.ENTITIES.191), and its represented `LootTemplate::Process`-style chance/group/count rolls now use a RNG derived from the session-owned runtime RNG instead of ad-hoc `thread_rng`.
