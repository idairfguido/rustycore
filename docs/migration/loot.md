...
- [ ] **#LOOT.11** `LootItem::get_ui_type_for_player` → `LootSlotType` decision tree (M). Pure C++ decision helper and represented `LootResponse` item consumption are covered by #NEXT.R8.ENTITIES.292. Represented creature loot now carries `_lootMaster` and `roundRobinPlayer` equivalents through #NEXT.R8.ENTITIES.293; full closure still requires canonical shared `Loot` ownership and per-player `GetPlayerFFAItems()` state.
