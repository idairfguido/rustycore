...
- Fishing loot is represented for ready bobbers: fish/junk opens consume `LootTemplates_Fishing` with C++ parent-area fallback and default zone `1` (#NEXT.R8.ENTITIES.322), and represented fishing-hole pool search now mirrors the C++ nearest `LookupFishingHoleAround(20.0f + CONTACT_DISTANCE)` plus `fishingHole.radius` gate (#NEXT.R8.ENTITIES.323). Remaining gaps are canonical bobber ownership, real area lookup, real fishing skill mutation and canonical `GameObject::m_loot`.
