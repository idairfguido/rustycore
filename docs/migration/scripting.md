...
- [ ] **#SCRIPTING.4** Implement the name→script-id lookup table loaded from `creature_template.ScriptName` / `gameobject_template.ScriptName` / `spell_script_names` / `areatrigger_scripts`. The C++ `ScriptNameContainer`-style interner now exists in `wow-data`, and template-store names can be collected deterministically; direct DB loader coverage for `spell_script_names` / `areatrigger_scripts` and `wow-script` consumers still need wiring. (M)
