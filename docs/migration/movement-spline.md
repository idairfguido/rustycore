...
- [ ] **#MOVE-SPL.15** Port `MoveSplineInit::Launch` (validate → commit `MoveSpline::Initialize` → send packet → return spline ID) + `Stop`. First core slice is done in `wow_movement::MoveSplineInit`: builder shape, `MoveTo/MovebyPath`, initial point correction, `enter_cycle`, movement flag forward/backward/root handling, selected/custom velocity with C++ speed clamp, `MoveSpline::Initialize`, returned duration, and `Stop` reinitialization with `MoveSplineFlag::DONE` plus tolerance 2. Remaining work: full Unit-owned launch/broadcast, transport transform, speed selection from real Unit movement mode, and all setters/callers. (H)
