...
- `get_realm_entry_json` usa el helper compartido `wow_core::select_ipv4_address_for_client_like_cpp` y `scan_local_ipv4_networks_like_cpp` para la ruta Unix IPv4; el helper genérico Unix IPv6 existe en `wow-core`, pero el caller sigue usando direcciones de realm IPv4 hasta ampliar la resolución/almacenamiento de candidatos.
...
- `world-server` transporta también `(RealmHandle::GetAddress(), Name, NormalizedName)` a `WorldSession`; `AuthResponse.VirtualRealms` y `CMSG_QUERY_REALM_NAME` responden con los nombres del snapshot, y address desconocida devuelve `RESPONSE_FAILURE` como C++ `RealmList::GetRealmNames`.
...
- ✅ fixed 2026-06-13: `WorldSession::handle_query_realm_name` respondía siempre `"Trinity"` y `AuthResponse.VirtualRealms` usaba `"RustyCore"`; ambos flujos ahora consumen `RealmList::GetRealmNames`-equivalent desde el snapshot `realmlist`.
...
- [x] **#REALM.13** Propagar `realmlist.name/NormalizedName` al `WorldSession` y usarlo en `AuthResponse.VirtualRealms` + `CMSG_QUERY_REALM_NAME`, con failure para addresses no presentes como C++ `RealmList::GetRealmNames`. (L)
...
| `Trinity::Net::SelectAddressForClient` | `wow_core::select_ip_address_for_client_like_cpp` / IPv4 wrapper + `scan_local_ip_networks_like_cpp` + callers `select_realm_ip_str` / `get_address_for_client` | Unix IPv4/IPv6 priority order and interface scan ported; callers still store/use IPv4 realm addresses; Windows parity pending |
