...
| `sWorld->UpdateSessions(1)` final flush | `WorldSessionShutdownFlushLikeCpp(diff=1)` ack marker after `KickAll` proves task-owned sessions drained prior shutdown commands before DB/offline cleanup | ⚠️ partial (#WS.15: no global session owner, no direct `WorldSession::Update`/erase/delete pass yet) |
