...
- **`MaxCoreStuckTime` config**: Rust now reads the direct C++ key/default through `max_core_stuck_time_secs_like_cpp` / `max_core_stuck_time_ms_like_cpp`, including `0` as the future disable value. It is still not wired to a live anti-freeze task.
- **`MinWorldUpdateTime` config**: Rust now reads the direct C++ key/default through `min_world_update_time_ms_like_cpp`, and the C++ sleep/update decision is represented and tested in `world_update_loop_step_like_cpp`, including the "increment counter before sleep" detail. It is not yet the top-level production cadence. Session loops still use `Duration::from_millis(50)`, while canonical/legacy map runtime paths use `CONFIG_INTERVAL_MAPUPDATE`.
- **`World::IsStopped()` / `World::StopNow(code)` / `World::GetExitCode()`**: `WorldRuntimeStateLikeCpp` now owns the stop flag, exit code and loop counter; shutdown signals call `stop_now_like_cpp(SHUTDOWN_EXIT_CODE)`; critical runtime task failures / unexpected exits call `stop_now_like_cpp(ERROR_EXIT_CODE)`; and `main` returns the stored process exit code. Remaining gap: the full top-level `WorldUpdateLoop` owner is still missing, so the counter is not yet driven by the production loop.
...
- **`ScanLocalNetworks`**: Unix IPv4/IPv6 host-interface scanning is now shared through `wow-core`; Windows parity and caller-level IPv6 realm address storage remain open.
...
- **`KickAll` + `UpdateSessions(1)` flush at shutdown**: partial — worldserver now tracks authenticated active sessions in `ActiveWorldSessionRegistryLikeCpp`, a minimal Rust analogue of C++ `World::m_sessions`, and shutdown enqueues `KickLikeCpp("World::KickAll")` to every active session before DB/offline cleanup using non-blocking `try_send`. It then queues `WorldSessionShutdownFlushLikeCpp(diff=1)` and waits for bounded acknowledgements so sessions prove they drained prior shutdown commands and report whether they are disconnecting. After the ack phase, shutdown waits on registry unregister notifications so task-owned sessions can run cleanup and leave the active-session map before DB/offline cleanup. The final C++ `UpdateSessions(1)` owner/removal pass is still missing until the global session-update owner can drive `WorldSession::update/process_pending` and erase finished sessions directly.
...
- **`sWorldSocketMgr.StopNetwork()`**: Rust now explicitly aborts the realm and instance listener tasks after the `KickAll`/shutdown-flush/unregister-drain phase and before DB/offline cleanup. This stops new accepts like C++ `WorldSocketMgr::StopNetwork`; full accepted-socket draining remains task-owned.
...
| `Trinity::Net::ScanLocalNetworks()` | `wow_core::scan_local_ip_networks_like_cpp` + `select_ip_address_for_client_like_cpp` for Unix IPv4/IPv6; Windows TODO | — |
...
| `Trinity::Net::ScanLocalNetworks()` | `get_address_for_client` + shared Unix IP scan/selection helper; caller still uses IPv4 realm addresses and Windows TODO | ⚠️ partial |
...
| `sWorld->KickAll()` (save + send logout) | `ActiveWorldSessionRegistryLikeCpp` tracks authenticated sessions and `kick_all_sessions_like_cpp` enqueues `SessionCommand::KickLikeCpp("World::KickAll")` for each during shutdown | ⚠️ partial (#WS.15: no queued-login list; final flush below still missing) |
| `sWorld->UpdateSessions(1)` final flush | `WorldSessionShutdownFlushLikeCpp(diff=1)` ack marker after `KickAll` proves task-owned sessions drained prior shutdown commands, followed by bounded wait for active-session unregister before DB/offline cleanup | ⚠️ partial (#WS.15: no global session owner, no direct `WorldSession::Update`/erase/delete pass yet) |
| `sWorldSocketMgr.StopNetwork()` | `stop_world_network_like_cpp` explicitly aborts realm + instance listener tasks after session flush and before DB/offline cleanup | ⚠️ partial (accepted sockets/session tasks remain task-owned; no socket-manager drain) |
...
**Partially represented.** `FreezeDetectorLikeCpp` implements the same counter-stall decision as C++ `FreezeDetector::Handler`, and `WorldRuntimeStateLikeCpp` owns the counter. The production anti-freeze task is still not started because the Rust server does not yet have the final `WorldUpdateLoop` that increments the counter. If the runtime hangs today (DB query stuck, scheduler livelock, deadlock), only an external supervisor would catch it, and no `sd_notify(WATCHDOG=1)` is being emitted.
