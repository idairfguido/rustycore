...
- **`KickAll` + `UpdateSessions(1)` flush at shutdown**: partial — worldserver now tracks authenticated active sessions in `ActiveWorldSessionRegistryLikeCpp`, a minimal Rust analogue of C++ `World::m_sessions`, and shutdown enqueues `KickLikeCpp("World::KickAll")` to every active session before DB/offline cleanup using non-blocking `try_send`. The final C++ `UpdateSessions(1)` flush/removal pass is still missing until the global session-update owner can drive `WorldSession::update/process_pending` directly.
