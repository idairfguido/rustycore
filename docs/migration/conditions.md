...
| Mount aura display | `crates/wow-data/src/spell.rs`, `crates/wow-world/src/session.rs` | `SPELL_AURA_MOUNTED` can now use retained `SpellEffect` misc fields plus Mount/MountXDisplay condition filtering and random usable-display selection to apply/remove represented mount display and `UNIT_FLAG_MOUNT` deltas, with creature_template_model fallback and represented VehicleKit/pet/collision side-effect requests; exact VehicleKit packets/seat runtime and PetMode packet emission remain pending. |
