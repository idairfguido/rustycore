...
| Mount aura display | `crates/wow-data/src/spell.rs`, `crates/wow-world/src/session.rs` | `SPELL_AURA_MOUNTED` can now use retained `SpellEffect` misc fields plus Mount/MountXDisplay condition filtering and random usable-display selection to apply a represented mount display delta; vehicle kit, pet control changes, creature-template fallback display remain pending. |
