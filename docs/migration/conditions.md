...
| Mount aura display | `crates/wow-data/src/spell.rs`, `crates/wow-packet/src/packets/pet.rs`, `crates/wow-packet/src/packets/vehicle.rs`, `crates/wow-world/src/session.rs` | `SPELL_AURA_MOUNTED` can now use retained `SpellEffect` misc fields plus Mount/MountXDisplay condition filtering and random usable-display selection to apply/remove represented mount display and `UNIT_FLAG_MOUNT` deltas, with creature_template_model fallback, owner `SMSG_MOVE_SET_VEHICLE_REC_ID`/`SMSG_SET_VEHICLE_REC_ID` packets for represented VehicleKit create/remove, owner `SMSG_MOVE_SET_COLLISION_HEIGHT` on mount/dismount, and `CMSG_MOVE_SET_VEHICLE_REC_ID_ACK` movement-info validation; `SMSG_PET_MODE` shape is ported, but exact VehicleKit seat/accessory runtime, broadcast-to-set `SMSG_MOVE_UPDATE_COLLISION_HEIGHT`, exact model collision-height calculation, and PetMode session emission remain pending. |
