...
- `wow-data::MountStore` now loads `Mount.db2` plus C++ hotfix rows and builds the C++ `_mountsBySpellId` index. `CollectionMgr::LoadAccountMounts` parity is represented by filtering account mount rows through `GetMount(spellId)`, keeping the accepted account mount flags in session, and applying `CMSG_MOUNT_SET_FAVORITE` with the C++ partial `SMSG_ACCOUNT_MOUNT_UPDATE` response. `Mount.PlayerConditionID` has a represented session helper for the C++ `CollectionMgr::AddMount` use/learn gate. `MountCapability.db2`, `MountTypeXCapability.db2`, and `MountXDisplay.db2` are also loaded with hotfix overlays and C++ grouping/order semantics. The pure `Unit::GetMountCapability` selection is represented and `WorldSession` now wires it to loaded area/map stores, current area/map, visible auras, known spells, `SKILL_RIDING` loaded from `character_skills`, live movement `SWIMMING`, and represented C++ `ZLiquidStatus` bits. Terrain/grid/vmap still needs to own refreshing that liquid status from map data.
