...
| Mount aura display | `crates/wow-data/src/spell.rs`, `crates/wow-data/src/creature_display.rs`, `crates/wow-packet/src/packets/pet.rs`, `crates/wow-packet/src/packets/vehicle.rs`, `crates/wow-world/src/session.rs` | `SPELL_AURA_MOUNTED` can now use retained `SpellEffect` misc fields plus Mount/MountXDisplay condition filtering and random usable-display selection to apply/remove represented mount display and `UNIT_FLAG_MOUNT` deltas, with creature_template_model fallback, owner `SMSG_MOVE_SET_VEHICLE_REC_ID`/`SMSG_SET_VEHICLE_REC_ID` packets plus `SMSG_ON_CANCEL_EXPECTED_RIDE_VEHICLE_AURA` for represented VehicleKit create/remove, `Unit::GetCollisionHeight()` recalculation from `CreatureDisplayInfo/CreatureModelData`, owner `SMSG_MOVE_SET_COLLISION_HEIGHT` plus broadcast-to-set `SMSG_MOVE_UPDATE_COLLISION_HEIGHT` on mount/dismount, represented `SMSG_PET_MODE` disable/enable emission when a session pet GUID is known, and `CMSG_MOVE_SET_VEHICLE_REC_ID_ACK` movement-info validation; exact VehicleKit seat/accessory runtime, full pet runtime ownership, and mount pet resummon execution remain pending. |
