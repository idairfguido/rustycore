...
- `wow-data::MountStore` now loads `Mount.db2` and builds the C++ `_mountsBySpellId` index. `CollectionMgr::LoadAccountMounts` parity is represented by filtering account mount rows through `GetMount(spellId)`, and `Mount.PlayerConditionID` has a represented session helper for the C++ `CollectionMgr::AddMount` use/learn gate. `MountCapability.db2`, `MountTypeXCapability.db2`, and `MountXDisplay.db2` are also loaded with C++ grouping/order semantics. The pure `Unit::GetMountCapability` selection over computed runtime state is represented; wiring that to live area/liquid/aura/riding state remains separate work.
