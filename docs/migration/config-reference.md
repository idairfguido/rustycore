...
| Hot-path security keys unread/partial | unread: `Warden.Enabled`; partial: `PacketSpoof.*` ban persistence + exhaustive opcode table, `MaxOverspeedPings` RBAC-exact bypass, `SocketTimeOutTime{,Active}` RBAC idle bypass. `SessionAddDelay` is C++-loaded but has no consumer in this legacy tree |
