...
- [ ] **#CONFIG.6** Port the remaining security-critical keys first: `Warden.{Enabled,…}` (gated on Warden module being ported) and finish `PacketSpoof.*` exhaustive opcode-table parity plus cross-session ban eviction. `SessionAddDelay` is loaded by C++ but has no consumer in the current legacy tree, so Rust should not invent runtime behavior for it unless another upstream/branch proves a consumer. `PacketSpoof.{Policy,BanMode,BanDuration}` is now read and AntiDOS log/kick/ban-row persistence is represented in `WorldSession::update` + `process_pending`; C++ `World::BanAccount` also kicks all affected online sessions, which remains pending, and the exhaustive table audit remains pending. `MaxOverspeedPings` is now validated, read by `world-server`, and enforced in `WorldSocket::HandlePing` style (`CMSG_PING` faster than 27s increments the counter; `0` disables; kick after `> maxAllowed`), but the exact `RBAC_PERM_SKIP_CHECK_OVERSPEED_PING` exemption remains pending until RBAC is represented. `SocketTimeOutTime{,Active}` is now read and enforced after wow-config's C++-like `/1000` conversion and active-only keepalive reset. `WrongPass.*` is now enforced in bnet REST login by optionally logging wrong-password attempts, incrementing `failed_logins`, inserting BNet account/IP autobans, and resetting failed-login count at the threshold, matching C++. `ChatFlood.{MessageCount,MessageDelay,AddonMessageCount,AddonMessageDelay,MuteTime}` is now read and enforced in represented chat handlers, but RBAC-exact exemption and DB persistence of newly assigned mutes remain pending. Ship the corresponding handler-side enforcement. (complejidad: H)
...
| Hot-path security keys unread/partial | unread: `Warden.Enabled`; partial: `PacketSpoof.*` cross-session ban eviction + exhaustive opcode table, `MaxOverspeedPings` RBAC-exact bypass, `SocketTimeOutTime{,Active}` RBAC idle bypass. `SessionAddDelay` is C++-loaded but has no consumer in this legacy tree |
...
10. **Anti-cheat / anti-spam partial**: `PacketSpoof.*` is now read and represented for AntiDOS log/kick decisions in `WorldSession::update`, and `Policy=2` persists account/IP ban rows during `process_pending`. C++ `World::BanAccount` still evicts all affected online sessions and the exhaustive opcode-table parity remains pending. `MaxOverspeedPings`, `SocketTimeOutTime{,Active}`, `WrongPass.*`, and `ChatFlood.*` are now represented for their current paths, but RBAC-exact exemptions remain pending for some checks. Severity: **critical** for any realm exposed to the public internet until the ban/RBAC gaps are closed.
