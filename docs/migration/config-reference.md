...
- ❌ ~588 of ~612 `worldserver.conf` keys are unread. Examples grouped by criticality:
  - **Security-critical (silently default to Rust hardcoded values)**: `MaxOverspeedPings`, `PacketSpoof.{Policy,BanMode,BanDuration}`, `Warden.*`, `WrongPass.*` (only on bnet side), `SocketTimeOutTime{,Active}`, `SessionAddDelay`. `ChatFlood.*` is now read/enforced for represented chat, with RBAC-exact exemption and DB persistence still pending.
...
10. **Anti-cheat / anti-spam unread**: `MaxOverspeedPings`, `PacketSpoof.*`, `WrongPass.*` (world side), `SocketTimeOutTime{,Active}`. `ChatFlood.*` is now represented for chat-session counters, but without the remaining keys enforced **the server still lacks rate-limiting on speed exploits, kicking on packet spoofing, idle-disconnect, and failed-login lockout**. Severity: **critical** for any realm exposed to the public internet.
