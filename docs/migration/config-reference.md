...
6. **`PacketSpoof.Policy = 1` is the default in TC** — log + kick. RustyCore now reads `PacketSpoof.{Policy,BanMode,BanDuration}` and represents AntiDOS log/kick/ban escalation, including account/IP ban persistence and C++-style online-session eviction. The per-opcode limit table is mirrored from `WorldSession::DosProtection::GetMaxPacketCounterAllowed`.
...
| Hot-path security keys unread/partial | unread: `Warden.Enabled`; partial: `MaxOverspeedPings` RBAC-exact bypass, `SocketTimeOutTime{,Active}` RBAC idle bypass. `PacketSpoof.*` is represented for the current AntiDOS path. `SessionAddDelay` is C++-loaded but has no consumer in this legacy tree |
...
10. **Anti-cheat / anti-spam partial**: `PacketSpoof.*` is now read and represented for AntiDOS log/kick decisions in `WorldSession::update`, and `Policy=2` persists account/IP ban rows during `process_pending` plus C++-style online-session eviction. The opcode limit table is mirrored from C++ `DosProtection::GetMaxPacketCounterAllowed`. `MaxOverspeedPings`, `SocketTimeOutTime{,Active}`, `WrongPass.*`, and `ChatFlood.*` are now represented for their current paths, but RBAC-exact exemptions remain pending for some checks. Severity: **critical** for any realm exposed to the public internet until the RBAC gaps are closed.
